...
MEASURED ON ibkr-paper, 2026-09-09 06:20 UTC. Both automated lanes had been UNARMED since 09-06:
...
put it in that state, and either one alone is enough:
  ...
  2. `_load()` takes "the first instrument that carries them". ibkr-paper's cache held BOTH windows:
...
#: The two real windows off ibkr-paper's book on 2026-09-09. Disjoint, which is what makes the bug
...
def test_a_STALE_instrument_does_not_hide_a_FRESH_one_in_the_same_cache():
    """Both windows were in ibkr-paper's cache. The calendar answered from the stale one and refused a
...
def test_the_window_is_RE_READ_when_the_venue_ADVANCES_it():
    """THE ONE THAT COST THREE DAYS. ibkr-paper's engine answered from a window ending 09-04 while the
...
def test_next_fire_ALSO_sees_the_advanced_window():
    ...
    — the one that was dead on ibkr-paper — unproven."""
    box = {"info": _STALE_INFO}
    cal = VenueCalendar(lambda: _mutable_cache(box))
...
def test_the_refusal_NAMES_THE_WIDEST_window_the_cache_could_describe():
    ...
    which is exactly the wrong-diagnosis half of the ibkr-paper incident."""
    cal = VenueCalendar(lambda: _cache([_STALE_INFO, _FRESH_INFO]))
