"""`claims_invariant` must IMPORT on a kumo-trading-strategies pin that predates its predicates (#903).
...
predicate is absent". That is the shape that crash-looped ibkr-paper's engine boot tonight (#902).
...
def test_the_predicates_are_KUMO_STRATEGIES_objects_not_a_copy():
    ...
    home = real_module(_PGRUNNER).__name__
    assert over_claimed.__module__ == home and own_ceiling.__module__ == home
...
def test_with_the_pin_present_nothing_changed():
    ...
    test in test_claims_invariant.py keeps pinning that it is kumo-trading-strategies' and not a copy."""
    import api.claims_invariant as mod
    importlib.reload(mod)
    from api.claims_endpoint import build_breaches
