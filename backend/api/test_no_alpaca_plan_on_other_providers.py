...
THE DEFECT, measured on ibkr-paper-retired 2026-08-28 02:45 SGT, from the instance's OWN log, 87 times:
...
def test_the_fixture_can_express_the_defect():
    ...
    This is the kumo-trading-strategies truncation-invariance failure one level out: a test that asserts an
...
def test_ALPACAS_CAP_IS_NOT_WHAT_DECIDES_a_non_alpaca_providers_ticks():
    """THE MEASURED FAILURE. 87 symbols on ibkr-paper-retired were denied live trade/quote by Alpaca's
...
def test_the_IBKR_tick_planes_are_refused_BY_THE_VENUES_OWN_DECLARATION_not_by_a_cap():
    ...
    assert ticks == [], (
        f"{len(ticks)} tick subscriptions went to a venue that declares it serves none — on ibkr-paper "
