...
def _installed():
    try:
        ...
        pytest.skip(f"installed kumo-trading-strategies does not carry {ma.CONTRACT_MODULE} — the mirror cannot be "
...
def _market_view_or_skip(*names: str):
    ...
    if missing:
        pytest.skip(f"installed kumo-trading-strategies market_view predates the three-state contract (missing {missing}) — "
...
def _skip_only_until_the_expiry_when_INVESTIGATE_is_absent(mv) -> None:
    ...
    if _today() < _INVESTIGATE_SKIP_EXPIRES:
        pytest.skip(f"installed kumo-trading-strategies {rev} has no SelfAction.INVESTIGATE (ks#182 unmerged as of "
    ...
    pytest.fail(f"as of 2026-09-11 ks#182 was unmerged and INVESTIGATE was absent from every pin in force; it is "
                ...
                f"kumo-trading-strategies {rev} STILL has no SelfAction.INVESTIGATE — either ks#182 stalled or this "
...
def test_the_action_union_spans_BOTH_upstream_enums():
    ...
    if self_action is None:
        pytest.skip("installed kumo-trading-strategies has no SelfAction yet (l21wvpmj is adding STAND_DOWN)")
...
def test_cockpits_NOTIFY_target_agrees_with_the_contracts_transition_for_the_same_pair():
    ...
    folded RISK_OFF → RISK_ON under LIQUIDATE into `entries_unblocked`; kumo-trading-strategies 80f6eae added
...
def test_the_action_union_carries_investigate_and_RESTRICTS_TRADING_is_implied_by_the_installed_reduces():
    """kumo-trading-strategies added `SelfAction.INVESTIGATE` (the high tail: above its own envelope — look,
    ...
    assert not hasattr(ma, "REDUCES"), "one name per question — `reduces` is kumo-trading-strategies' word for a smaller book"
