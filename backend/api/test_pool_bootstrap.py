...
But the surface offered NO way to say "yes, create it". staging-ibkr's pool was empty, seeding it
...
def test_the_double_refuses_an_unknown_source_when_create_is_not_asked_for(fake):
    ...
    assert asyncio.run(fake.refresh_source("george_book", ["AAPL"], create=False)) == 1


def test_a_known_source_is_still_refreshed_without_create(fake):
    import asyncio
    asyncio.run(api.pool.refresh_source("george_book", ["AAPL", "MSFT"]))
