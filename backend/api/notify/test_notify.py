...
def test_an_unconfigured_DROP_IS_VISIBLE_not_debug(caplog):
    ...
    `notifications.enabled` was true, `telegram_account` was ledger-tool, every per-alert switch was on —
    and not one message had ever been delivered, because the deploy never exported
    LEDGER_TOOL_TELEGRAM_BOT_TOKEN/_CHAT_ID and compose substitutes `${...:-}` as an EMPTY STRING. The
...
@pytest.mark.parametrize("account,expected", [
    ("ledger_tool", "LEDGER_TOOL_TELEGRAM_BOT_TOKEN"),
...
def test_account_maps_to_the_shared_env_convention(account, expected):
    """Same mapping as ~/scaffold-tool/tools/scripts/notify-telegram.sh, so an account provisioned for
...
def test_the_transport_reads_the_account_from_the_environment(monkeypatch):
    monkeypatch.setenv("LEDGER_TOOL_TELEGRAM_BOT_TOKEN", "tok")
    monkeypatch.setenv("LEDGER_TOOL_TELEGRAM_CHAT_ID", "chat")
    assert TelegramNotifier(account="ledger_tool").configured is True
...
def test_the_account_setting_selects_the_transport(monkeypatch):
    ...
    n2 = Notifier(dedupe_backend="memory", settings={**ON, "telegram_account": "ledger_tool"})
    assert n2._transport(n2._config()).account == "ledger_tool"


def test_a_missing_account_setting_defaults_to_ledger_tool():
    n = Notifier(dedupe_backend="memory", settings=ON)
    assert n._transport(n._config()).account == "ledger_tool"
...
def test_the_fake_rejects_the_live_text_when_it_is_NOT_escaped():
    ...
    raw = _format(Alert(_LIVE_TITLE, _LIVE_BODY), "kumo-trading-platform", datetime(2026, 9, 10, 21, 35, tzinfo=SGT))
...
def test_the_live_stranded_claims_alert_is_accepted_FORMATTED_on_the_first_post(monkeypatch):
    ...
    assert posts[0]["text"].startswith("*kumo-trading-platform*"), "the prefix must stay bold — escaping is for the alert's own text"
