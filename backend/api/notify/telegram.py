"""Telegram delivery, ported from fintrack's `tools/scripts/notify-telegram.sh`.

That script has been carrying fintrack's job alerts for a while and its behaviours are the reason this
is a port rather than a fresh implementation:
  ...
  CRITICAL IGNORES QUIET HOURS. The engine being down is worth waking up for. fintrack draws the same
  line at job failure.
...
so the cockpit can send as an existing bot without minting a new one. fintrack reads the macOS
keychain directly, which a container cannot, so the deploy exports the pair as the Alpaca keys are:

    export FINTRACK_TELEGRAM_BOT_TOKEN=$(security find-generic-password -a fintrack -s FINTRACK_TELEGRAM_BOT_TOKEN -w)
    export FINTRACK_TELEGRAM_CHAT_ID=$(security find-generic-password -a fintrack -s FINTRACK_TELEGRAM_CHAT_ID -w)
...
def env_keys(account: str) -> tuple[str, str]:
    ...
    Mirrors `~/vibe-hq/tools/scripts/notify-telegram.sh`: uppercase, hyphens stripped. Keeping the
...
#: the WHOLE message when either opens an entity that never closes — measured on paper 2026-09-10:
...
def _format(alert: Alert, prefix: str, now: datetime) -> str:
    """fintrack's shape, so both projects' alerts read alike in the same client."""
...
class TelegramNotifier:
    ...
    def __init__(self, token: str | None = None, chat_id: str | None = None,
                 prefix: str = "kumo-cockpit", account: str = "fintrack") -> None:
