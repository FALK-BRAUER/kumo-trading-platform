...
Operator alerts over Telegram (#197). Ported from fintrack's `tools/scripts/notify-telegram.sh`, so
...
**Which bot is a setting; its credentials are not.** `telegram_account` (default `fintrack`) maps to
`{UPPER}_TELEGRAM_BOT_TOKEN` / `_CHAT_ID` with hyphens stripped — the same convention as
`~/vibe-hq/tools/scripts/notify-telegram.sh`, so any account already provisioned for another project
works here with no new bot and no new keychain entry.

Default is `fintrack`: the cockpit's alerts are trading alerts and belong beside fintrack's rather
than in a second channel to remember to check.

Secrets stay environment-only. fintrack reads the macOS keychain directly; a container cannot, so the
