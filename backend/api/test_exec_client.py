...
#
# This is the second cause of one symptom. Making `broker_equity` a method (kumo-strategies ccea7b4)
...
def test_no_strategy_reads_a_broker_account_KEY_WE_DO_NOT_PUBLISH():
    ...
    `portfolio_value`; kumo-strategies b0593af corrected them and the marker XPASSed, which is the
    ...
    root = next(
        (c for c in (p.parent / "kumo-strategies" / "src" / "kumo_strategies" for p in here.parents)
         if c.is_dir()),
        None,
    )
    if root is None:
        pytest.skip(
            "the kumo-strategies checkout is not beside this repo; the installed venv copy is "
