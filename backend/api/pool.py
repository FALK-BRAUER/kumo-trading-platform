...
The pool is what MOMENTUM ranks each session: a union of sources (george_book, my_watchlist) with an
...
sources carry a symbol — are strategy-side domain logic and belong in kumo-strategies. Re-deriving
...
async def refresh_source(name: str, symbols, *, detail: str | None = None,
    ...
    a fresh instance has to be able to create its first source: staging-ibkr could not seed an empty
