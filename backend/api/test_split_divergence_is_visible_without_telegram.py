...
MEASURED on staging2, 2026-09-09 19:10 SGT, by running `api/split_divergence.py::split_divergence`
...
`if self._enabled():` (alerts.py:1043), which reads `notifications.enabled` — FALSE on staging2. One
...
def test_the_fixture_carries_a_HELD_position_with_NO_claim_that_must_not_be_reported():
    ...
    flags every unattributed position, which on staging2 would be all 17."""
    from api.split_divergence import split_divergence
