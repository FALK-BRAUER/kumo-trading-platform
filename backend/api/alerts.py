...
def health_conditions(health: dict[str, Any]) -> dict[str, Alert]:
    ...
    for d in health.get("reconcile_drift", []) or []:
        ...
        if not sym:
            ...
            continue
        broker, cockpit = d.get("broker_qty", 0), d.get("platform_qty", 0)
...
class AlertsService:
    ...
    async def _announce_lanes_bleeding(self) -> None:
        """A lane going to cash under a TRADING label (#1098): NO DECISION for N sessions while the
        protection plane's stop-outs shrink its book and nothing enters. MOMENTUM-002 8 -> 2 names,
        BCTROT-004 12 -> 5, over 2026-09-15..17 on ibkr-paper, every surface green.

        ONE fold, shared with `/health` (`api.lanes_bleeding.scan_bleeding`) — the detection lives on
        the health path where it runs with notifications off; this is the DELIVERY half, keyed per
        lane so a second lane pages while the first is muted. Critical: the absence of decisions is
        indistinguishable from a healthy quiet lane unless it says so.
        """
        try:
            out = await self._lanes_bleeding_impl()
        except Exception as exc:                                        # noqa: BLE001
            self._check_failed("lanes_bleeding", exc)
            return
        if out.get("status") != "ok":
            # A read that failed is the CHECK failing, counted toward BROKEN_CHECK_POLLS — silence
            # from an unreadable journal must not read as "no lane bleeds".
            self._check_failed("lanes_bleeding", RuntimeError(str(out.get("error") or "unreadable")))
            return
        self._check_ok("lanes_bleeding")
        from api.lanes_bleeding import LaneBleed

        for lane in out.get("lanes") or []:
            b = LaneBleed(**{k: lane[k] for k in ("strategy_id", "venue_exits", "own_entries",
                                                    "no_decision_sessions", "since", "blocked")})
            # ONE key derivation — `LaneBleed.alert_key` — not an f-string beside it (review).
            await self._n.send(
                b.alert_key(),
                Alert(title=f"{b.strategy_id}: BLEEDING — going to cash under a TRADING label",
                      body=f"{b.line()}\n\n"
                           f"Protection keeps selling; nothing buys. The lane has not decided for "
                           f"{b.no_decision_sessions} sessions: {b.blocked}",
                      critical=True))

    async def _lanes_bleeding_impl(self) -> dict:
        """Split so the send path can be driven without a database (the `_never_ran_impl` idiom)."""
        from api.db.engine import session_factory
    ...
    async def _announce_stranded_claims(self) -> None:
        ...
        # `platform_qty`, `models.py` pins it) — the same key `account_from_positions` produces.
    ...
    async def _announce_pool_unlisted(self) -> None:
        ...
        directly, and never touches HTTP. `ledger_book` refreshed at 10:32 on 2026-08-29, an hour
        ...
        if verdict.status != "ok":
            ...
            #
            # BY-DESIGN ABSENCE IS SAID ONCE; A BROKEN ORACLE IS SAID EVERY POLL. On ibkr-paper-retired
        ...
        while True:
            try:
                ...
                if self._enabled():
                    ...
                    # A lane going to cash under a TRADING label (#1098). Reads Postgres, so inside
                    # the gate; its own `_check_failed` guard, like split divergence.
                    await self._announce_lanes_bleeding()
