...
Two consequences, both pinned here:
  ...
  * A VENUE THAT NEVER RESERVES SKIPS THE WAIT. IB has no reservation concept
    ...
    False when the answer was None — which is exactly why every ibkr-paper-retired exit died: "no
...
def test_a_venue_that_never_reserves_SKIPS_the_wait():
    """Exits are dead on ibkr-paper-retired precisely here: the venue reserves nothing, the availability
...
def test_the_builder_hands_the_reservation_fact_to_the_feed():
    ...
    assert assigns, (
        ...
        "ibkr-paper-retired's exits keep waiting on a reservation that does not exist"
    )
    sources = [
