...
kumo-strategies owns `NautilusBroker.exit()`, which calls it. Between them sits a seam with no type,
...
Three separate defects lived in that gap in one evening, every one of them with BOTH suites green:
  ...
  1. `exit()` called `self.strategy.release_for_exit(...)`. `broker.strategy` is the ROTATION strategy,
     ...
     intent row was journalled. Cockpit's test bound the real function to a host; kumo-strategies'
  ...
  3. A commit landed on the kumo-strategies branch reasoning at length that the strategy_id argument
     was "load-bearing" — written against a signature that no longer exists.

WHY THE VENV CANNOT BE TRUSTED TO ANSWER THIS
----------------------------------------------
`pyproject.toml` pins kumo-strategies by git SHA, but `deploy/Dockerfile.backend:38-39` installs it from
...
def test_the_installed_broker_actually_has_the_exit_path():
    ...
    assert hasattr(broker, "exit"), (
        ...
        "`uv pip install --python .venv/bin/python --no-deps --reinstall ../../kumo-strategies`"
    )
...
def test_the_call_binds_against_cockpits_real_signature():
    ...
    try:
        ...
        pytest.fail(
            f"kumo-strategies calls release_for_exit with {len(call.args)} positional arg(s) and "
...
def test_the_lane_the_caller_passes_NEVER_BECOMES_THE_OWNER_FILTER():
    ...
    QUESTION. As of `cb5ee79` the deployed kumo-strategies DOES pass it (`nautilus/broker.py:162`),
    ...
    assert "strategy_id" in accepts, (
        "cockpit no longer accepts `strategy_id`, but the deployed kumo-strategies passes it "
