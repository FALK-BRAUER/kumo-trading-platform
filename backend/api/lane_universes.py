...
readers of ONE list, never two lists. Measured on ibkr-paper 2026-09-10: `QC27_UNIVERSE` held 138 names,
...
#: (gate key, symbol key, default symbols) per fixed-symbol lane. These lanes do not have a ranking
#: universe to read from settings, but an enabled lane can still submit orders for its fixed legs. The
#: connector needs those contracts in the same load_contracts union or IBKR can price the decision and
#: then fail at submit with a missing instrument.
FIXED_LANE_UNIVERSES: tuple[tuple[str, str, tuple[str, ...]], ...] = (
    ("SMHGLD_ENABLED", "SMHGLD_SYMBOLS", ("SMH", "GLD")),
)
...
def fixed_lane_symbols(values: dict, symbols_key: str, default: tuple[str, ...]) -> list[str]:
    """One fixed-symbol lane's legs, normalised, sorted, de-duplicated.

    Missing or empty means the schema/lane default, matching SMHGLD's builder. These are not budget
    universe fields: the point is to tell IBKR which instruments an enabled fixed lane may submit.
    """
    raw = (values or {}).get(symbols_key) or list(default)
    return sorted({norm(s) for s in raw if str(s).strip()})
...
def enabled_lane_universe_symbols(values: dict) -> dict[str, list[str]]:
    ...
    values = values or {}
    out = {ukey: universe_symbols(values, ukey)
           for gkey, ukey in LANE_UNIVERSES if bool(values.get(gkey))}
    out.update({skey: fixed_lane_symbols(values, skey, default)
                for gkey, skey, default in FIXED_LANE_UNIVERSES if bool(values.get(gkey))})
    return out
