...
(engine_node.py:993). staging-ibkr has none — so every `if self._http is None: return` was a whole
...
# refused to even TRY on the one stack (staging-ibkr) where the typed path is the only path.
# --------------------------------------------------------------------------------------------------


def test_reducing_orders_are_read_TYPED_when_there_is_no_rest_client():
    """The staging-ibkr shape: an exec client, no AlpacaHttpClient. The venue holds one resting
