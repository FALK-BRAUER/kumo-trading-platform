...
class RedisConsumer:
    ...
    def __init__(self, cfg: FeedConfig | None = None) -> None:
        ...
        #: last-gap value: measured on an Alpaca paper instance 2026-09-11 00:34, the bridge does not drop once during
    ...
    def health(self) -> dict:
        ...
        return {
            ...
            # A LIVE FRAME WITHOUT THE KEY IS ALSO None (#1099, #1105 review). The engine always
            # publishes the key — `{}` when no lane — so its absence means an engine that predates it:
            # the new-api/old-engine seconds at every deploy. Defaulting that to `{}` made "could not
            # ask" read as "no lane is here", which `/strategies.engine_frame` then reported as ok.
