...
Measured on paper 2026-08-29: `george_book` was refreshed at 10:32, an hour after #663 deployed, and
...
class _Pool:
    ...
    async def effective(self):
        ...
        self._i += 1
        return {s: PoolEntry(s, ("george_book",), False, {}) for s in sorted(syms)}
...
class _FakeDb:
    ...
    def __init__(self, sources, pins=(), excludes=()):
        self._rows = {
            PoolSource: [PoolSource(source="george_book", symbol=s, meta={}) for s in sources],
...
def test_the_sweep_names_a_symbol_the_engine_refresh_wrote_that_the_venue_does_not_list(
    ...
    """The #723 subject: `george_book` refreshed at 10:32 through the engine, no HTTP hop, no
...
def test_absence_BY_DESIGN_warns_once_while_a_BROKEN_ORACLE_warns_every_poll(
    ...
    On staging-ibkr `search is None` is permanent and correct. A WARNING every 30s for that is ~2,880
...
def test_a_symbol_its_GATE_refused_is_not_marked_as_though_it_had_paged(monkeypatch, _isolate):
    ...
    for name in ("_announce_slots", "_announce_never_ran", "_announce_external_positions",
                 "_announce_split_divergence", "_announce_stranded_claims"):
