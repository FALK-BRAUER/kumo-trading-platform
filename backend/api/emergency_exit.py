"""`on_emergency_exit` — the callback target kumo-strategies' emergency exit calls (#922, a8504dc).
...
def _outcome_class(outcome_type):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"and the installed kumo-strategies has none: {type(exc).__name__}: {exc}") from exc
