...
-------------------------------------------------------------------------------
Operator, 2026-08-15: "why would you even look at the adapter. you just need any name if it contains 003
...
Operator, 2026-08-15, after a collision: "maybe the strategy # should be managed by cockpit and prefix by
strategy?"

Yes, and the collision is the argument. QC345 was assigned tag `003` by taking the next free number
here, while kumo-strategies#32 had already published a strategy called `BCTROT-003`. Both were
...
_IMMOVABLE: dict[str, str] = {
    ...
    # (kumo-strategies runtime/nautilus/qc345_rotation.py), so cockpit allocating anything else would
    # simply disagree with the thing that runs.
    #
    # This tag had an argument. kumo-strategies#32 is titled "BCTROT-003", so I reserved 003 for a
...
class StrategyEntry:
    ...
    #: that when the adapter exposes its own (kumo-strategies#157) a disagreement is representable.
    cadence: str
...
REGISTRY: tuple[StrategyEntry, ...] = (
    ...
    StrategyEntry(
        # QC27 Tech Momentum with Inverse Volatility (kumo-strategies#33, #63). Tag 005 from
    ...
    StrategyEntry(
        # CRSISHORT (kumo-strategies#123, cockpit#858). Tag 006 from `next_free_tag`. THE FIRST
        # LANE THAT HOLDS THE SHORT SIDE — `ownership.SHORT_PERMITTED` names it and only it.
        # Registered in SHADOW: its order path (a resting short LIMIT, kumo-strategies#131) does
        ...
        protection_reason=(
            ...
            "(kumo-strategies crsi_short.py); a broker stop on a short is a BUY the protection plane "
            "cannot attribute to a lane, and a second exit rule beside the adapter's would race it."),
    ),
    StrategyEntry(
        # SMHGLD (kumo-strategies#177, cockpit#953/#965). Tag 007 from `next_free_tag`. The
