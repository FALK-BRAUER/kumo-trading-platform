...
live protective stops, read out of the durable cache 2026-08-22:
    ...
    PROT-SELL-BDX-XNYS-00000002    MOMENTUM-002    a lane's OWN trailing stop
...
def test_an_UNATTRIBUTABLE_order_is_REFUSED():
    """Cannot attribute, cannot justify. kumo-trading-strategies' own ids are `kumo-{sha1}` with the owner
...
# ==================================================================================================
# OWNER ALONE IS TOO WIDE (review, kumo-trading-platform-ibkr session). `may_cancel` on the owner only lets any
...
# ==================================================================================================
# ATTRIBUTION NEEDS A SECOND SOURCE (review, kumo-trading-platform-ibkr session).
#
...
def test_a_FOREIGN_id_stays_unattributable():
    ...
    assert owner_from_prefix("kumo-3f9a1c") is None          # kumo-trading-strategies' own convention
    assert owner_from_prefix("8f2c1e77-3a4b-4f0e") is None   # an id ALPACA minted (#242)
    assert owner_from_prefix("") is None


# ==================================================================================================
# THE THIRD SHAPE, AND IT REFUSED THE ONLY STOP ON THE BOOK (review, kumo-trading-platform-ibkr session).
#
...
def test_a_lanes_OWN_stop_reached_through_the_exit_path_is_NOT_refused():
    """The live case: PROT-SELL-BDX-XNYS-00000002, owned by MOMENTUM-002, 55 shares."""
    assert may_cancel_order(owner="MOMENTUM-002", canceller=PROTECTION_OWNER,
                            row=_row(coid="PROT-SELL-BDX-XNYS-00000002"),
...
def test_OUR_STOP_PREFIXES_covers_every_prefix_cockpit_ACTUALLY_MINTS():
    ...
    and kumo-trading-strategies demonstrated today why that is not enough: their
