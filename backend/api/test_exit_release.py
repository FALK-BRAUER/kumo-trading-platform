...
The shares were held. `long_market_value` was 75,000.00 at the time. They were RESERVED, by the
...
Cockpit #358. The matching half (`NautilusBroker.exit`, `pgrunner.py:816`) is kumo-trading-strategies'.
"""
...
def test_the_cancel_confirm_FILTER_and_the_authorisation_identity_are_DIFFERENT_arguments():
    """THE 2026-08 DEFECT, kept impossible by construction (review, kumo-trading-strategies).
...
def test_a_NAMED_lane_becomes_the_authorisation_identity_and_NOT_the_filter(monkeypatch):
    """The path kumo-trading-strategies is about to start using, driven before they do.
...
def test_WITHOUT_a_named_lane_the_proxy_concession_stays_ON(monkeypatch):
    """Today's behaviour, and the discriminating half. kumo-trading-strategies does not pass a lane yet, so the
