...
`exec_position_state` and `exec_action_log` are declared by KUMO-TRADING-STRATEGIES and created by its
...
on the first attempt to stand up `ibkr-paper-retired`.
...
kumo-trading-platform issue 486.
"""
...
def test_every_migration_altering_a_FOREIGN_table_guards_for_its_absence():
    ...
    assert not unguarded, (
        ...
        f"how ibkr-paper-retired failed to bootstrap. Guard with `sa.inspect(op.get_bind()).has_table(...)`"
    )
