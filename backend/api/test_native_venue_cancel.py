...
def test_cancel_order_can_be_built_without_the_cache():
    ...
    cmd = CancelOrder(
        trader_id=TraderId("PLATFORM-001"),
...
class _Recorder:
    ...
    def __init__(self, rows, with_client=True):
        ...
        self.trader_id = TraderId("PLATFORM-001")
...
def test_ANOTHER_LANES_resting_order_is_NOT_cancelled_by_the_venue_sweep():
    ...
    # OWNED BY ANOTHER LANE. Not prefix-attributable either — kumo-trading-strategies mints `kumo-{sha1}` — so
...
def test_a_named_lane_may_cancel_ITS_OWN_stop_through_the_sweep():
    ...
    This is also the realistic case the moment kumo-trading-strategies threads `req.strategy_id` through: a lane
