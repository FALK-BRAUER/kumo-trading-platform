...
def test_drift_is_keyed_per_symbol():
    ...
    h = {**OK_HEALTH, "reconcile_drift": [{"symbol": "HSBC", "broker_qty": 0, "cockpit_qty": -93},
                                          {"symbol": "MET", "broker_qty": 105, "cockpit_qty": 0}]}
...
def test_drift_wording_names_the_direction():
    ...
    h2 = {**OK_HEALTH, "reconcile_drift": [{"symbol": "AAPL", "broker_qty": 100, "cockpit_qty": 0}]}
...
def test_drift_is_not_critical():
    ...
    h = {**OK_HEALTH, "reconcile_drift": [{"symbol": "X", "broker_qty": 0, "cockpit_qty": 1}]}
...
def test_a_resolved_condition_is_cleared_so_it_can_alert_again(monkeypatch):
    ...
    tx = FakeTx()
    node = FakeNode(drift=[{"symbol": "HSBC", "broker_qty": 0, "cockpit_qty": -93}])
    svc = _svc(node, tx)
    ...
    for _ in range(2):                           # must stay clean for _DOWN_POLLS_BEFORE_CLEAR
        asyncio.run(svc._announce_health())
    node._drift = [{"symbol": "HSBC", "broker_qty": 0, "cockpit_qty": -93}]
...
def test_a_single_poll_blip_does_not_count_as_recovery(monkeypatch):
    ...
    monkeypatch.setattr("api.app._probe_subsystems", probe)
    tx = FakeTx()
    drift = [{"symbol": "HSBC", "broker_qty": 0, "cockpit_qty": -93}]
...
def test_a_drift_row_with_no_symbol_is_skipped_not_collapsed():
    ...
    h = {**OK_HEALTH, "reconcile_drift": [{"broker_qty": 0, "cockpit_qty": -1},
                                          {"symbol": "MET", "broker_qty": 1, "cockpit_qty": 0}]}
...
def _drift_row(sym="HPE"):
    return {"symbol": sym, "broker_qty": 36, "cockpit_qty": 0}
