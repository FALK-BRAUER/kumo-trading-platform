"""A lane that STARTED but cannot DECIDE must not read as healthy (#438, kumo-trading-strategies 7de760a).
...
def test_cockpit_requires_EXACTLY_what_the_INSTALLED_strategies_package_reports():
    ...
    The first version of this test read cockpit's own tuple and quoted kumo-trading-strategies' in PROSE:
    ...
    directions, and the reverse — kumo-trading-strategies adding a probe cockpit never requires — is the one
    nothing else would ever notice.

    THIS COUPLES THE SUITE TO THE INSTALLED PACKAGE VERSION, DELIBERATELY. `pyproject.toml` pins
    kumo-trading-strategies by git URL, and a pinned URL plus a test that never looks is how version skew ships:
