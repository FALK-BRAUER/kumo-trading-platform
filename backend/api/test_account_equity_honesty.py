...
# it cannot run. Both halves measured on ibkr-paper-retired, 2026-08-24.
# ---------------------------------------------------------------------------------------------
...
IB_SGD_NET_LIQUIDATION = 1_000_016.15   # the live ibkr-paper-retired account, read off its AccountState


def test_the_real_ib_account_reports_a_single_SGD_leg_and_is_still_published():
    """THE ACTUAL IBKR PAPER ACCOUNT. Measured, not assumed:

        AccountState(account_id=INTERACTIVE_BROKERS-DUPTEST02, account_type=MARGIN,
                     base_currency=None,
                     balances=[AccountBalance(total=1_000_016.15 SGD, locked=9_000.00 SGD,
                                              free=991_016.15 SGD)])
...
def test_an_ambiguous_multi_currency_account_publishes_NOTHING():
    ...
    s, published, _w = _ib_strat(positions_open=[object()],
                                 legs={SGD: 1_000_016.15, Currency.from_str("EUR"): 12_000.0})
    UiFeedStrategy._publish_account(s)
    assert not [p for kind, p in published if kind == "account"]
...
def test_ALPACA_IS_NOT_IN_THE_ALLOW_LIST_AND_MUST_NOT_BE():
    ...
    Measured on a live instance account 2026-08-24:
