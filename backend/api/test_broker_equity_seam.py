...
`exit []` — and submitted ZERO orders. The only trace anywhere:
    ...
    [ERROR] COCKPIT-001.QC345: QC345: rebalance 2026-08-21 failed: TypeError: 'NoneType' object is not callable
...
The property returns None when `self._broker_account` is empty. That is not rare: kumo-cockpit#382,
...
-----------------------------
Both halves live in kumo-strategies, so neither is cockpit's to fix. But cockpit is what DIES: the
...
A test on either kumo-strategies class alone would pass — each is internally consistent. The defect
...
FIXED UPSTREAM in kumo-strategies `e26556f` — `broker_equity` is a plain method on QC345 now. This file
