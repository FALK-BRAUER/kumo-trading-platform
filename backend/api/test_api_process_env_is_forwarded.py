...
MEASURED ON staging2, 2026-09-09. `instance.env` declares `KUMO_ORDERS_ARMED=false`. The engine
container has it. The api container does not:

    $ docker exec kumo-staging2-engine-1 printenv KUMO_ORDERS_ARMED   -> false
    $ docker exec kumo-staging2-api-1    printenv KUMO_ORDERS_ARMED   -> exit 1
