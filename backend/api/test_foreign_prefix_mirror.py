"""kumo-strategies must recognise every client-order-id prefix THIS repo mints (#748).
...
A prefix cockpit mints and kumo-strategies does not recognise is a fill that lane will treat as its
...
kumo-strategies until that pin bumps. That is the coupling surfacing, which is what a mirror across a
...
def _their_prefixes():
    """The prefixes kumo-strategies recognises — or a decision about WHY it cannot say.
    ...
    try:
        ...
        pytest.fail(
            ...
            f"they move book state. Bump the `kumo-strategies` pin in pyproject.toml"
        )
...
def test_the_fixture_matches_what_this_repo_actually_mints():
    ...
    # writing the prefix in a COMMENT and broken by deleting one — the shape kumo-strategies has a
...
def test_kumo_strategies_recognises_EVERY_prefix_cockpit_mints():
    ...
    assert not missing, (
        f"kumo-strategies does not recognise {sorted(missing)} as foreign. Cockpit stamps these "
        ...
        f"prefix in kumo-strategies' `order_provenance.FOREIGN_ORDER_PREFIXES` and bump the pin"
    )
...
def test_our_own_stop_prefixes_are_a_SUBSET_of_what_they_recognise():
    ...
    assert not missing, f"stop prefixes unknown to kumo-strategies: {sorted(missing)}"
