...
MEASURED 2026-08-31, on two running stacks against two different brokers:
    ...
    paper   (Alpaca)  PROT-SELL-AEM-XNYS-763905c0  AEM.XNYS  ACCEPTED
    staging (IBKR)    PROT-SELL-AEM-XNYS-763905c0  AEM.XNYS  REJECTED
...
with the engine reporting:
    ...
    cannot see (PROT-SELL-AEM-XNYS-763905c0). ... Exits will be rejected on `available: 0`
...
(COCKPIT-001 vs COCKPIT-STG), is already required to construct a node, and cannot be forgotten on a
...
PAPER, STAGING = "COCKPIT-001", "COCKPIT-STG"
...
def test_the_ACCESSOR_actually_returns_the_nodes_trader_id():
    ...
    class _Probe(UiFeedStrategy):
        @property
        def trader_id(self):
            return TraderId("COCKPIT-STG-001")

    s = _Probe.__new__(_Probe)
    assert s._trader_id_str() == "COCKPIT-STG-001"
