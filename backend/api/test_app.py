...
_ROW = {"symbol": "AFL", "sources": ["george_book"], "provenance": "george_book",
...
def test_pool_reports_source_health(client, monkeypatch) -> None:
    ...
    _fake_pool(monkeypatch, [_ROW], sources=[
        {"name": "george_book", "status": "ok", "symbol_count": 70,
         "refreshed_at": "2026-08-05T18:24:38+00:00", "stale": False, "detail": None}])

    sources = client.get("/pool").json()["sources"]

    assert sources[0]["name"] == "george_book" and sources[0]["stale"] is False
...
def test_the_distribution_primitive_is_REACHABLE() -> None:
    ...
    while $44,889.50 sat unallocated and the only route to funding them was waiting for MOMENTUM to sell.
...
def test_health_REPORTS_split_divergence_with_notifications_OFF(client, monkeypatch):
    ...
    MEASURED on staging2 2026-09-09: eleven pairs, 1,504 shares, BCTROT-004 claiming positions the
