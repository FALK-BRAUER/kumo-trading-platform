...
that `kumo-trading-platform/instances` is the authority on what an instance IS. The defect is that the
...
ROOT = pathlib.Path(__file__).resolve().parents[2].parent / "kumo-trading-platform/instances"


def _makefile() -> str:
    mk = ROOT / "Makefile"
    if not mk.exists():
        pytest.skip("kumo-trading-platform/instances is not checked out beside this repo")
    return mk.read_text()
