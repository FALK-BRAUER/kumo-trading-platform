...
(kumo-strategies 78e634c), so an IBKR-only instance can finally satisfy the requirement instead of
...
def test_this_module_does_NOT_hard_require_kumo_strategies_at_import():
    ...
    153 of 184 test files do not import kumo-strategies; the 30 that do are a separate job holding
    the private-repo credential. `api.engine_node` imports this module, so a module-level
    kumo-strategies import here makes the whole engine unimportable without it — and the `backend`
    ...
    Every other kumo-strategies import in this codebase is function-local for the same reason. This
