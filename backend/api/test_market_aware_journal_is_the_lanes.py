...
Measured on an Alpaca paper instance 2026-09-11 after the b930bd2 recreate: `/health.market_aware.journal_absent` lists ALL
...
`Strategy` subclass from kumo-trading-strategies) exposes NEITHER — its runner holds the journal, and the
contract ships `session_journal()` (`runtime/nautilus/contract.py:709`) written for exactly this
defect (kumo-trading-platform issue 587: "QC345 returned None and wrote nothing for the lane's entire life").
...
def test_a_RAISING_resolver_is_journal_BROKEN_not_journal_absent():
    """Cross-review of #958 (ibkr-paper session): a resolver that RAISES and a lane that genuinely has
