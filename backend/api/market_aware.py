...
`.acts` IS THE SINGLE DERIVATION. kumo-trading-strategies' `Verdict.acts` / `Assessment.acts` (f5895d1) say
...
#: divergence and kumo-trading-strategies 80f6eae added the kind — same words. The mirror adopted it.)
...
#: `investigate` (kumo-trading-strategies, 2026-09-11): the lane sits ABOVE its own envelope — beating its
...
#: a truthy action read as "reduce something" would flatten a WINNING lane (issue 144).
...
#: acting on this make the book SMALLER?" for kumo-trading-strategies' own enum, and EXIT_ONLY caps a book
