...
ALPACA = VenueFacts(
    ...
    facts=(
        ...
        fact(
            ...
            "issue 48 / kumo-trading-platform issue 358 — FSM and VCTR, 2026-07",
        ...
        fact(
            ...
            "venue behaviour relied on by kumo-trading-strategies broker.py:78 (reduce_only=is_exit)",
        ...
        fact(
            "price_adjustments", frozenset({"raw", "split"}), Provenance.MEASURED,
            "2026-09-18, paper key, /v2/stocks/NVDA/bars 1Day sip: 2024-06-07 close 1208.88 under "
            "adjustment=raw, 120.89 under split (== IB's TRADES bar), 120.54 under all (dividends "
            "folded in). Volume scales with the split too (41.2M raw -> 412.4M split).",
            "#1124: CRSISHORT was measured on split-adjusted bars; 29 of its 130 names split inside a "
            "200-day window, so on raw bars the lane reads a +-90% day that never traded. `all` is "
            "refused at the HTTP door — a third series nobody measured.",
        ),
    ),
)
...
IBKR = VenueFacts(
    ...
    facts=(
        ...
        fact(
            ...
            "balances=[AccountBalance(total=1_000_016.15 SGD, locked=9_000.00 SGD, free=991_016.15 SGD)]",
        ...
        fact(
            ...
            "kumo-trading-platform issue 430 — IB does not reserve shares against a resting order the way Alpaca "
        ...
        fact(
            "price_adjustments", frozenset({"split"}), Provenance.MEASURED,
            "ibkr-paper gateway, IB TRADES 1-DAY bars: NVDA 2024-06-07 close 120.89 (memory "
            "`ib-daily-bars-rth-and-shortable`, 2026-09-13) — the same number Alpaca gives under "
            "adjustment=split; IB's historical API has no adjustment parameter for TRADES.",
            "#1124: a lane needing split-adjusted bars reads this set, never the provider's name. "
            "A request naming `raw` is answered with split bars here; there is no raw series to serve.",
        ),
    ),
)
