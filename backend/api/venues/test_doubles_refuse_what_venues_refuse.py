...
def test_a_currency_the_account_does_not_hold_is_REFUSED():
    ib = account_for("INTERACTIVE_BROKERS", net_liquidation=1_000_016.15, available_funds=991_016.15)
    with pytest.raises(ValueError, match="no USD balance"):
        ib.balance_total(USD)
    assert ib.balance_total(SGD) == 1_000_016.15, "the currency it DOES hold must still answer"
...
def test_balance_total_means_THE_SAME_THING_on_both_venues_since_588():
    ...
    ib = account_for("INTERACTIVE_BROKERS", cash=1.0, net_liquidation=1_000_016.15)
    assert 73_393.33 != 103_500.35 and 1.0 != 1_000_016.15, "fixture cannot tell cash from net-liq"

    assert alpaca.balance_total(USD) == 103_500.35, "Alpaca must now put NET LIQUIDATION in total"
    assert ib.balance_total(SGD) == 1_000_016.15, "IB puts NET LIQUIDATION in total"
...
def test_portfolio_equity_is_STILL_unusable_on_alpaca_but_now_it_OVERSTATES():
    ...
    pinned kumo-trading-strategies (grepped 2026-08-27). This test pins the arithmetic, not a live path.
    """
...
def test_ib_does_not_reserve_shares_that_way():
    """Why kumo-trading-platform issue 430 is venue-conditional rather than a bug to fix everywhere."""
