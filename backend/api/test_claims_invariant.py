"""The claims ledger may not claim more than the account holds (#437, kumo-strategies#66).

MEASURED ON THE LIVE PAPER BOOK, 2026-08-22. Three of eight symbols breach RIGHT NOW:
...
The predicate is kumo-strategies' (`over_claimed`, c2358d1) and is deliberately NOT reimplemented here
...
def test_the_fixture_contains_BOTH_a_consistent_symbol_and_a_breach():
    ...
    the fixture breached, an implementation that flagged everything would pass. kumo-strategies'
...
def test_the_predicate_is_KUMO_STRATEGIES_and_not_a_second_implementation():
    ...
    assert "from kumo_strategies" in src, (
        "cockpit has reimplemented the claims invariant instead of importing kumo-strategies' "
