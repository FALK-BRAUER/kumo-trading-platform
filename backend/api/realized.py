...
# `positions_closed()` for good. ibkr-paper, 2026-09-09: MPC.XNYS-MANUAL-001 closed for +575.51, reopened,
...
def _window(legs: list[ClosedLeg], start_ns: int | None, end_ns: int, *, partial_open: int,
    ...
    return {
        ...
        # (ibkr-paper: 2026-09-03). Stated on every window so no figure reads as a lifetime number.
...
def _fill_class(order, ev) -> str | None:
    ...
    # and the cockpit's identity rides in the tag (ibkr-paper, measured 2026-09-14). Ours.
    ...
    if str(getattr(order, "strategy_id", "")) == "EXTERNAL":
        ...
        # writes a second later at the lane's fill px (ibkr-paper 09-11 12:08:17, fr2467iv's table).
...
def split_boot_pairs(kernel_fills: list[dict], *, since: str) -> tuple[list[dict], dict[str, int], dict[str, float]]:
    ...
    A BOOT PAIR is what IB reconciliation writes per held name at boot (ibkr-paper 2026-09-11: a SELL
