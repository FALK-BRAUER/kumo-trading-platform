...
MEASURED ON staging2. Two boots, two outcomes, and the difference is a third of a second:
...
def test_the_gateway_healthcheck_probes_the_PORT_THE_ENGINE_CONNECTS_TO():
    ...
    The gateway serves the engine on the socat port (`KUMO_IBG_PORT`, 4004 on staging2), while the
...
def test_the_start_period_still_covers_an_IBC_login():
    """IBC took 24s from launch to `Configuration tasks completed` on staging2's 2026-09-09 boot and
