...
staging-ibkr could not boot: `build_calendar(require_exchange=True)` refuses the holiday-unaware
...
def _strategies_calendar():
    ...
    Module-level would make `api.engine_node` — which imports this — hard-require kumo-strategies at
    ...
    Every other kumo-strategies import in this codebase is function-local for the same reason.
    """
...
class VenueCalendar:
    ...
    def _load(self, d: date | None = None) -> dict:
        ...
        life of the process is guaranteed to age out. staging2 booted on 2026-09-09 and answered every
