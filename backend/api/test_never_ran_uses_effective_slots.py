...
Measured 2026-08-29 on paper: every `*_SLOTS` setting is `[]`.
    ...
    the LANES read []  ->  "keep the built-in schedule"  (slots_from_settings: `if not raw: return
                            default`; kumo-trading-strategies does the same, `if not got: return`)
...
def test_the_LIVE_shape_would_now_fire():
    ...
    assert out == {
        ...
        # upstream adapter's signature default (issue 177), stated literally here.
...
def test_every_builtin_matches_what_the_LANE_actually_declares():
    ...
    kumo-trading-strategies' own OPEN_OFFSET_MINUTES docstring records the SAME literal drifting once
    ...
    for sid in BUILTIN_SLOTS:
        ...
        if declared is None and sid == "CRSISHORT-006":
            # The adapter lives in a kumo-trading-strategies revision this venv may not carry yet (#858);
