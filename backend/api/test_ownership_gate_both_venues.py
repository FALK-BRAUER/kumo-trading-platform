...
`test_every_exec_client_is_gated` pinned the PROPERTY. This file does the same for ownership:
  ...
  * protective stops are skipped by IDENTITY, not by `is_reduce_only` — the flag-based skip made the
    guard blind to every kumo-strategies lane exit (all submitted `reduce_only=is_exit`), and on
...
silence. Coverage and scope reviewed cross-repo by the kumo-strategies session (codex out until
...
def test_a_reduce_only_LANE_EXIT_IS_asked__the_flag_is_not_a_pass():
    """THE REVIEW FINDING THAT CHANGED THE SCOPE. kumo-strategies submits every lane exit and cover
...
def test_EVERY_exec_client_factory_declares_BOTH_gates():
    for name, factory in _every_exec_factory():
        assert getattr(factory, "gates_budget", False), (
            f"{name} does not declare the budget gate — this is exactly how staging-ibkr ran with no "
            f"per-strategy cap (#782)")
        assert getattr(factory, "gates_ownership", False), (
            f"{name} does not declare the ownership guard — this is how staging2 ran with no guard "
...
# ==================================================================================================
# THE SEAM: the two declarations of a lane's side — cockpit's and kumo-strategies' — must agree
# ==================================================================================================
def test_the_two_side_declarations_agree():
    """`api.ownership.SHORT_PERMITTED` is keyed by LANE ID; kumo-strategies' `POSITION_SIDE` by
    ...
    literal `strategy_id=` kwarg. A seam test: it needs the installed kumo-strategies (ks#146 shape).
    """
    import ast
    import importlib
    import pathlib
    ...
    # When kumo-strategies is not installed, `conftest.py` deselects this whole file by source scan
    ...
    for path in sorted(root.glob("*.py")):
        ...
        for lane, name in sorted(pairs):
            ...
            # kumo-strategies contract raises on that absence, and so does this.
