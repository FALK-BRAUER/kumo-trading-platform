...
-----------------------
The DEPLOYMENT half: no sidecar, no data mount, no path env, and fintrack's tooling mounted read-only
...
def test_the_ENGINE_no_longer_MOUNTS_A_TOOLING_DIRECTORY_to_compute_the_rotation():
    ...
    off, `KUMO_FINTRACK_TOOLS=/nonexistent` read as deliberate configuration, and staging-ibkr showed
    ...
    tooling = [str(v) for v in vols if "fintrack/tools" in str(v)]
    assert not tooling, (
    ...
    declared = [l for l in _COMPOSE.read_text().splitlines()
                if "KUMO_FINTRACK_TOOLS" in l and not l.lstrip().startswith("#")]
    assert not declared, (
