...
LEDGER_TOOL = "/ledger-tool/tools"

#: SCOPED TO THE TESTS THAT ACTUALLY NEED THE MOUNT. This was a module-level `pytestmark`, so all six
#: tests skipped on any host without `/ledger-tool` — which is every developer machine and every CI run,
#: leaving the converters covered by nothing. Only `build_payload` imports ledger-tool's module; the
#: converters are pure and must run everywhere.
needs_ledger_tool = pytest.mark.skipif(
    not __import__("os").path.exists(f"{LEDGER_TOOL}/rotation_read.py"),
    reason="ledger-tool tools are not mounted in this container",
)
