"""The `on_emergency_exit` adapter — a callback target over `liquidate_lane` (#922, kumo-trading-strategies a8504dc).

The lane DECIDES (kumo-trading-strategies `strategies/emergency.py`); the owner of the positions ACTS. Cockpit
...
The outcome type lives upstream. Until it lands on kumo-trading-strategies main the adapter builds the outcome
...
def test_installed_shape_the_real_EmergencyOutcome_matches_the_double():
    em = pytest.importorskip("kumo_strategies.strategies.emergency",
                             reason="strategies/emergency.py (a8504dc, PR #169) not on the installed kumo-trading-strategies yet — adapter builds through a call-time import")
