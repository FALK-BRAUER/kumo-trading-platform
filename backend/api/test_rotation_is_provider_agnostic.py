...
no log. staging-ibkr renders today ONLY because its `KUMO_DATA=alpaca`; an instance taking data from
IBKR would have no market view and would say nothing about it.

That is the same silent-absence this feature was just rescued from: `KUMO_FINTRACK_TOOLS` could switch
...
def test_a_provider_that_cannot_serve_the_history_SAYS_SO():
    ...
    `return` is what made `KUMO_FINTRACK_TOOLS=/nonexistent` look deliberate for days."""
    body = _refresh_rotation_body()
    tree = ast.parse("class X:\n" + "\n".join("    " + l for l in body.splitlines()))
