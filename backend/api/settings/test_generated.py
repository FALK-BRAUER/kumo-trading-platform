"""Strategy settings domains generated from the kumo-trading-strategies dataclasses (#318/#320, upstream #32/#37).
...
def test_a_promoted_override_survives_upstreams_own_group_default():
    """Upstream now emits a group `default` built from the dataclass (kumo-trading-strategies 1209cac).
...
def test_setting_descriptions_are_short_and_impersonal():
    """2026-08-16: "shortent the setting text. They also sometimes are specifically for me.
    ...
    and are excluded — rewriting those belongs in kumo-trading-strategies.
    """
    import json
    import re
...
def test_a_not_operator_settable_group_is_OFFERED_read_only_even_when_upstream_cannot_walk_it():
    ...
    if not _config_declares_market_view():
        pytest.skip("installed kumo-trading-strategies QC345RotationConfig declares no market_view (the deployed pins) — nothing to offer")
...
def _upstream_can_walk_market_view() -> bool:
    """CAPABILITY, measured: kumo-trading-strategies cd35d6e added the Enum mapping; the deployed pins predate it.
...
def test_a_not_operator_settable_group_is_walked_with_every_field_from_the_dataclass():
    if not _upstream_can_walk_market_view():
        pytest.skip("installed kumo-trading-strategies predates cd35d6e (no Enum mapping): the group is offered as a "
...
def test_upstreams_editable_node_wins_when_it_stops_excluding(monkeypatch):
    if not _upstream_can_walk_market_view():
        pytest.skip("installed kumo-trading-strategies cannot walk MarketViewConfig: un-excluding it raises upstream")
    if _upstream_market_view_node() is not None:
        pytest.skip("installed kumo-trading-strategies emits market_view itself (read-only, cd35d6e) — there is no "
...
def test_a_read_only_group_still_resolves_so_the_operator_sees_the_declared_defaults():
    if not _config_declares_market_view():
        pytest.skip("installed kumo-trading-strategies QC345RotationConfig declares no market_view (the deployed pins) — nothing to resolve")
