"""Strategy settings domains, GENERATED from the kumo-trading-strategies config dataclasses (#318/#320).

Every other domain here is a `*.schema.json` file someone wrote. A strategy's parameters must not be,
and upstream's own reasoning is the argument (issue 32): `MomentumRotationConfig` gained
...
_STRATEGY_DOMAINS: dict[str, tuple[str, str, dict[str, Any]]] = {
    ...
    #
    # It was added here, and that was a mistake caught by the kumo-trading-strategies peer before it reached
    ...
    # Tracked on kumo-trading-platform issue 323.
    "qc345": (
        "kumo_strategies.strategies.qc345_rotation.config",
        "QC345RotationConfig",
        {
            # The live-feasible candidate promoted by kumo-trading-strategies PR #36. Measured over the
...
def _to_json_schema(node: dict, *, path: str, overrides: dict[str, Any]) -> dict:
    ...
    for group_name, group in (node.get("groups") or {}).items():
        ...
        # Upstream (kumo-trading-strategies cd35d6e) emits not-operator-settable groups itself, read-only,
    ...
    if path:
        ...
        #
        # Upstream now emits a group `default` itself (kumo-trading-strategies 1209cac) after this was
...
def _offer_read_only_groups(schema: dict, config_cls: type, *, schema_for) -> None:
    ...
    Since kumo-trading-strategies cd35d6e upstream EMITS the group itself, read-only, with its own reason —
