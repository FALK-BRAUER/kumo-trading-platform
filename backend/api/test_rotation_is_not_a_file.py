...
WHAT IT USED TO BE. A sidecar container shelled out to `ledger-tool/tools/rotation_read.py` on a host
...
NOTHING IS REIMPLEMENTED. ledger-tool's tool still computes the whole read — `ratio_bars`, `ichi`,
...
def test_the_bars_are_MEMOISED_per_session_not_refetched_every_tick():
    ...
    # alpaca-paper sat at 0 of 25 axes for fifteen minutes with ZERO RequestBars in that window.
    #
...
def test_rotation_is_served_as_a_PLANE_not_only_as_a_poll():
    """2026-08-21: "It should work similar to trend lines, watch, portfolio etc."
