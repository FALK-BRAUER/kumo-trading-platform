...
Three of these exist because kumo-strategies hit them first and told us. Inheriting their fixes is
...
def test_a_digest_of_nothing_is_REFUSED_not_returned(tmp_path):
    ...
    kumo-strategies hit this: a worktree that failed to materialise produced a convincing MISMATCH
...
def test_the_count_and_the_digest_describe_the_SAME_SET(tmp_path):
    ...
    kumo-strategies' `files()` and `digest_tree()` walked the tree separately. A mutation narrowing
...
def test_EVERY_shipped_file_is_hashed_not_only_python(tmp_path):
    ...
    kumo-strategies' first version hashed `*.py` alone while fifteen non-`.py` files shipped — a
