...
Operator, 2026-08-27: *"the instances project is defining the instance's config"* and *"staging has NO
...
The consequence was not cosmetic: staging-ibkr took its market data from Alpaca, which built an
...
# ==================================================================================================
# THE INSTANCE RULE — "staging has NO business with alpaca whatsoever" (Operator, 2026-08-27)
# ==================================================================================================
...
def _instance_env(name: str) -> dict:
    p = _REPO.parent / "kumo-cockpit-instances" / "instances" / name / "instance.env"
    if not p.exists():
...
def _feed_toml(name: str) -> dict:
    p = _REPO.parent / "kumo-cockpit-instances" / "instances" / name / "feed.toml"
    if not p.exists():
        pytest.skip(f"{p} not checked out beside this repo")
...
def test_staging_ibkr_declares_NO_alpaca_provider():
    ...
    env = _instance_env("staging-ibkr")
    assert env.get("KUMO_EXEC") == "ibkr", env
    assert env.get("KUMO_DATA") == "ibkr", (
        f"staging-ibkr declares KUMO_DATA={env.get('KUMO_DATA')!r} — it must have no Alpaca anything")
    assert _feed_toml("staging-ibkr")["data"]["provider"] == "ibkr", (
...
def test_staging_ibkr_names_NO_alpaca_credential_anywhere():
    ...
    root = _REPO.parent / "kumo-cockpit-instances" / "instances" / "staging-ibkr"
    if not root.exists():
    ...
    assert not offenders, (
        "staging-ibkr still declares Alpaca configuration:\n  " + "\n  ".join(offenders))
