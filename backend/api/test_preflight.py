...
WHY THE STRATEGY MAY NOT JUDGE ITSELF. kumo-strategies' own contract.py says it:
...
def test_equity_that_RAISED_fails():
    """kumo-strategies ccea7b4 — `broker_equity` was a @property, so `equity()` called its RESULT and
...
def test_equity_that_returned_NONE_fails_and_is_NOT_the_same_diagnosis_as_a_raise():
    """kumo-strategies b0593af — both QC lanes read `portfolio_value` from a message that publishes it
...
def test_a_DISABLED_lifecycle_fails_while_the_lane_is_enabled():
    ...
    row leaves it DISABLED", and an absent row reads as TRADING (Operator, 2026-08-19). A lane with no row
...
def test_OWNED_is_judged_against_WHETHER_THE_STRATEGY_HAS_EVER_FILLED():
    ...
    kumo-strategies 578fbb6: TECHIVOL read `broker.positions()` — the ACCOUNT book — as its own. It
...
def test_the_two_halves_of_the_probe_set_are_DISJOINT_and_complete():
    """BOUNDARY, set with kumo-strategies 2026-08-22.
    ...
    # `armed` joined 2026-08-22 with kumo-strategies 7de760a. It belongs on the STRATEGY side for the
    ...
    # take for itself.
    # `universe` joined 2026-08-29 with kumo-strategies#80 / a456c26, on the STRATEGY side by the
...
def test_a_MISSING_probe_says_WHOSE_bug_it_is():
    """A lane that stopped reporting `equity` is kumo-strategies' problem; a missing `budget` is cockpit
