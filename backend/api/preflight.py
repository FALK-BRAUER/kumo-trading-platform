...
THE STRATEGY OBSERVES. THE PLATFORM JUDGES. kumo-strategies' contract.py states the rule and this
...
#: WHO OBSERVES WHAT (boundary set with kumo-strategies, 2026-08-22).
#:
...
#: `armed` FIRST, matching kumo-strategies' `PREFLIGHT_PROBES`. It is the one capability cockpit
#: cannot observe from outside: a lane whose calendar never answered reads healthy on every other
#: probe while being unable to ever decide (kumo-strategies 7de760a).
REQUIRED_FROM_STRATEGY = ("armed", "equity", "price", "owned", "universe")
#: `universe` (kumo-strategies#80): {source, requested, resolved, unresolved, ambiguous}. Counts,
...
def judge_preflight(probes, *, enabled: bool, has_filled_this_session: bool) -> list[ProbeResult]:
    ...
    # different readers: a lane that stopped reporting `equity` is kumo-strategies' problem, a missing
...
def _judge_one(name, probe, by_name, has_filled) -> ProbeResult:
    ...
    v = probe.value
    if name == "armed":
        # A LANE THAT STARTED BUT CANNOT DECIDE. Before kumo-strategies 7de760a a lane whose calendar
    ...
    if name == "universe":
        # kumo-strategies#80. Counts, not a verdict — THE RELATIONSHIP IS THE DETECTOR:
