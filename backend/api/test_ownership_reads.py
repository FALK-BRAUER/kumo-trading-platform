...
`strategy_id=` flags them, and a guard that cries wolf gets switched off — which is how kumo-strategies'
...
def _blind_readers(files=_FILES) -> list[str]:
    ...
    SCOPED PER FUNCTION. A module-wide scan gave kumo-strategies a false positive on its first run:
