...
(`<root>/kumo-trading-platform/deploy` beside `<root>/kumo-trading-strategies`), against two scratch git repos.
"""
...
def _scratch(tmp_path: Path, *, pin_line: str, strategies_commits: int = 1, strategies_checkout: str | None = None):
    ...
    cockpit = root / "kumo-trading-platform"
    _repo(cockpit)
    ...
    strategies = root / "kumo-trading-strategies"
...
@pytest.fixture
def clean(tmp_path):
    deploy, shas = _scratch(tmp_path, pin_line='"kumo-trading-strategies @ git+https://github.com/X/kumo-trading-strategies.git@{sha}"', strategies_commits=1)
...
def test_ALLOW_UNPUSHED_does_NOT_waive_the_pin_gate(tmp_path):
    ...
    deploy, shas = _scratch(tmp_path, pin_line='"kumo-trading-strategies @ git+https://github.com/X/kumo-trading-strategies.git@deadbee"')
...
def test_a_parse_miss_on_the_pin_line_FAILS_CLOSED_with_a_message(tmp_path):
    ...
    deploy, _ = _scratch(tmp_path, pin_line="'kumo-trading-strategies @ git+https://github.com/X/kumo-trading-strategies.git@main'")
    rc, out = _run(deploy)
...
def test_a_checkout_AHEAD_of_the_pin_is_refused_without_a_loud_override(tmp_path):
    ...
    deploy, shas = _scratch(tmp_path, pin_line='"kumo-trading-strategies @ git+https://github.com/X/kumo-trading-strategies.git@{sha}"', strategies_commits=2)
    py = deploy.parent / "backend" / "pyproject.toml"
