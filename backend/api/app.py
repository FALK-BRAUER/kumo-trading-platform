...
async def _snapshot_for(node: Node, topic: Topic) -> dict | None:
    ...
    if topic.channel == "rotation":  # the rotation read, graded off our own bars (#384)
        # A PLANE, LIKE EVERY OTHER FACT THE UI READS. Operator, 2026-08-21: "It should work similar to
...
async def _split_divergence(node):
    ...
    merely whether it SENT. staging2 stood at eleven divergent pairs and 1,504 shares with every
...
async def get_strategies() -> dict:
    ...
    for entry in REGISTRY:
        ...
        rows.append({
            ...
            # value (kumo-strategies#157), when it arrives, can be compared rather than overwrite.
