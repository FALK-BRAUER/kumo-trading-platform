...
staging2, 2026-09-11 07:18:13Z (cockpit aeed851 / strategies 84d09d3): `TradingNode: STARTING`, then
...
not at risk; the fix is for the CLASS, and the value is chosen from staging2's numbers.
...
THE INPUT THE VALUE WAS DERIVED FROM: ~250 qualified contracts on staging2 (the CRSISHORT universe
...
#: The two boots that decide the value (staging2, 2026-09-11, ~250 contracts incl. 8 unresolvable).
_STAGING2_CLEAN_BOOT_S = 50.5
_STAGING2_FAILED_BOOT_S = 66.0
...
#: unconfigured timeout is not a decision, it is the 60 s that sat under staging2 until 2026-09-11.
...
def test_FIXTURE_nautilus_default_connect_timeout_is_SHORTER_than_staging2s_failed_boot():
    ...
    assert _nautilus_default("timeout_connection") < _STAGING2_FAILED_BOOT_S
...
def test_the_node_passes_timeout_connection_and_it_covers_the_failed_boot_with_margin():
    ...
    assert ours >= 2 * _STAGING2_FAILED_BOOT_S, f"{ours}s does not cover the ~66 s boot twice over"
    assert ours >= 3 * _STAGING2_CLEAN_BOOT_S
