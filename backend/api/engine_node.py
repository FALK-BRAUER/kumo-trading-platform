...
def _protection_coid(instrument_id: str, side: str, attempt: int = 0, lane: str = "",
    ...
    # 2026-08-31, paper and staging both minted PROT-SELL-AEM-XNYS-00000001 against two different
...
#:
#: NO LONGER A FILE. The payload used to be written by a sidecar shelling out to `ledger-tool/tools/
#: rotation_read.py` on a host mount and read back off disk. 2026-08-21: "it should not feed from a
...
#: classified ABSENT for that tick and stops gating publication. Measured 2026-08-27 on ibkr-paper-retired:
...
def _baseline_from_env(value: str | None) -> float:
    ...
    has to be TOLD, and the instance is what knows it: kumo-trading-platform/instances declares
    `ACCOUNT_BASELINE_EQUITY` per stack (paper's actual opening balance is 100,000; ibkr-paper-retired's is
...
# `realtime_symbol_budget` used to live here and were applied to every provider, so ibkr-paper-retired rationed
...
#: The attribute a lane carries WHY it is not armed, when the installed kumo-trading-strategies has one
    ...
    for sid, lane in siblings.items():
        ...
        if isinstance(v, bool):
            ...
            if isinstance(declared, dict) and declared.get("state"):
                ...
                # `arm_state` is built in kumo-trading-strategies and this repo does not own its types. A
...
def next_fire_by_lane(siblings: dict) -> dict:
    ...
    kumo-trading-strategies and codex, working independently, refuted it — QC345 is a MONTHLY rebalancer, and
    ...
    The mechanism this function exists for is still real — kumo-trading-strategies' MOMENTUM-002 lost its
...
def _single_currency_amount_and_ccy(balances: dict, *, prefer):
    ...
    place that knows which unit the number is in threw it away. On ibkr-paper-retired that published
    1,000,015.19 SGD as an unlabelled float beside USD position values, with `base_currency=None`
...
#: The pending-futures key under which the shortable plane's TABLE REFRESH rides beside the
#: per-instrument subscriptions (#1102 d). Not an InstrumentId, and never treated as one.
_SHORTABLE_REFRESH = "__shortable_refresh__"
...
class UiFeedStrategy(Strategy):
    ...
    def __init__(
        ...
        price_adjustments: frozenset[str] | None = None,
    ) -> None:
        ...
        #: Sibling strategies in THIS node, by StrategyId (#372, issue 49). Registering an id
        ...
        #
        # THE FALLBACK IS THE ONE kumo-trading-strategies ALREADY SHIPS. `build_calendar` selects
        ...
        # WHICH PRICE SERIES the provider's historical bars can be served in (#1124) — the spec's
        # declaration, forwarded by build_node. None = "never told us" (a hand-built double); a lane
        # that needs split-adjusted bars refuses that as undeclared, never reads it as raw.
    ...
    def _pace_lane(self, strategy) -> None:
        ...
        The lanes are kumo-trading-strategies runners built by cockpit. Their warmup — `momentum_rotation.py:297`,
    ...
    def _realtime_budget(self) -> float:
        ...
        evaluated on every provider. ibkr-paper-retired therefore rationed itself to 7 live symbols and
    ...
    def _after_definition(self, instrument_id: InstrumentId) -> None:
        ...
        if allow_realtime:
            ...
            # DOES NOT NO-OP — it issues `reqTickByTickData` and IB rejects it. ibkr-paper's boot of
    ...
    def _short_lanes(self) -> list:
        ...
        (`POSITION_SIDE`, kumo-trading-strategies sides.py: "short"), never by name."""
    ...
    def _shortable_catchup(self) -> int:
        ...
        for iid, fut in list(self._shortable_pending.items()):
            ...
            if exc is None:
                continue
            # `isinstance` FIRST: a Cython InstrumentId refuses `==` against a str with TypeError
            # (measured), and inside the observation wrapper that read as "no failures counted".
            if isinstance(iid, str) and iid == _SHORTABLE_REFRESH:
                # The table refresh (below) failed: the table stays as it was — stale reads None
                # per name and `stale` on /health, loud — and the next pass asks again.
                self._shortable_subscribe_failures += 1
                _log.error("shortable table refresh failed (%r) — table unchanged, retry next pass", exc)
                continue
        ...
        for lane in lanes:
            for iid in list(getattr(lane, "_iids", None) or ()):
                ...
                scheduled += 1
        # A PLANE WITH A TABLE REFRESHES IT FROM HERE (#1102 d, ks review of #1129). Alpaca's plane
        # loads its asset table on `subscribe`, and every name is subscribed once — so after the
        # boot pass nothing reloaded, at hour 27 the table went stale, every name read "no locate"
        # and only a restart recovered. This pass is the one periodic hook on the plane; IB's plane
        # has no table (tick 236 streams), so the verb is looked up, never assumed. The refresh is a
        # coroutine on the node loop like a subscription, and a failed refresh keeps the stale
        # table — which reads None per name and `stale` on /health, loud, until the next pass.
        refresh = getattr(self._shortable_plane, "refresh_if_stale", None)
        if refresh is not None and _SHORTABLE_REFRESH not in self._shortable_pending:
            self._shortable_pending[_SHORTABLE_REFRESH] = self._schedule_shortable(refresh())
        return scheduled
    ...
    def _resolve_instrument_id(self, symbol: str) -> str | None:
        ...
        iterated first, and on ibkr-paper that keyed the venue's CGAU stop `(CGAU.XNAS, SELL)` while the
    ...
    async def _reconcile_protection_inner(self, broker_rows, plan_protection, resolve, declared) -> None:
        ...
        try:
            ...
            # the ENTIRE reconciler dead on ibkr-paper-retired (no AlpacaHttpClient exists there) while every
            ...
            #
            # Measured on a live instance account (`scripts/probe_held_orders.py`): 269 orders, 9 returned by
        ...
        # OWN reducing side and an instrument-level answer cannot say which. Measured on a live instance
        ...
        # instrument outright. Measured on an Alpaca paper instance 2026-09-08: CRAK/LAND/PAGP/GMAB each had one lane's
        ...
        #
        # Measured on an Alpaca paper instance 2026-09-10 (`scripts/probe_trailing_replace.py --m1`): `type` is not
    ...
    # is the ACT behind the #873 LIQUIDATE condition and the target of kumo-trading-strategies' `on_emergency_exit`
    ...
    async def _handle_flatten_command(self, cid: str, payload: dict, entry_id: str) -> tuple[str, str]:
        ...
        try:
            ...
            # strategy; that route does not exist yet (issue 49) and pretending otherwise here
            # is what produced both failures.
            # ROUTE IT TO THE OWNER RATHER THAN REFUSING (issue 49).
            #
    ...
    async def _reserve_attach_command(
        ...
        *,
        ...
        #
        # ROUNDED, NOT REFUSED (2026-08-19: "if it is subdecimal you can round. no problem").
        #
    ...
    async def _handle_attach_manager_command(self, cid: str, payload: dict, entry_id: str) -> tuple[str, str]:
        ...
        human gate for automation (2026-08-02: no per-action confirm step once armed)."""
        if not self._orders_armed:
    ...
    def _market_aware_journal_of(self, lane):
        """The lane's journal, resolved by the LANE — `session_journal()` from kumo-trading-strategies'
        contract (`runtime/nautilus/contract.py`), written for exactly this defect (kumo-trading-platform issue 587):
    ...
    async def _venue_shares_available(self, instrument_id: str) -> Decimal | None:
        ...
        the read was broker-specific forever and answered permanent-None on ibkr-paper-retired. Alpaca
    ...
    async def _await_shares_available(
        ...
        # timeout and returned False: every ibkr-paper-retired exit died on a reservation the venue does not
    ...
    async def release_for_exit(self, instrument_id: str, qty, side: str = "SELL",
        ...
        were plainly held (`long_market_value` 75,000.00). They were RESERVED, by the `PROT-SELL-*`
        ...
        it is in the signature, and as of `cb5ee79` the deployed kumo-trading-strategies passes it on every
        ...
        Called from `NautilusBroker.exit()` (kumo-trading-strategies) on the exit-side SELL path only.
        """
        ...
        try:
            ...
            # measured on a live book, PROT-SELL-BDX-XNYS-00000002 owned by MOMENTUM-002. The flag
            ...
            #
            # `proxy` only while the lane is unknown: kumo-trading-strategies' `NautilusBroker.exit` does not
    ...
    async def _protection_status_note(
        ...
        saw this on LFXX while a PROT-SELL for the full 400 shares was resting the whole time.
    ...
    def _cancel_unprotectable_stops(self, now_ns: int) -> None:
        ...
        Measured on an Alpaca paper instance 2026-08-31: 17 of 23 live protective orders were in exactly that state, and
    ...
    def _canon_manager_params(self, handler, instrument_id: str, params: dict) -> dict:
        ...
        2026-08-19: "if it is subdecimal you can round. no problem."
    ...
    def _publish_equity_curve_from_cache(self) -> None:
        ...
        rendered "No account history yet" over an account holding $1,000,000 with months of history.
        ...
        persists it. Measured 2026-08-26: 280,000 events on paper, 22,000 on staging. The IBKR curve
    ...
    # and false about the world: `Account.events` had 22,000 AccountState events on the IBKR stack the
    ...
    async def _refresh_rotation(self) -> None:
        ...
        host tool and writing `rotation.json`, which the API read back. 2026-08-21: "it should
        ...
        try:
            ...
            # empty with nothing to retry it. Measured 2026-08-26: alpaca-paper sat at 0 of 25 axes for
    ...
    def _mark_broker_stop_prices(self, dtos) -> None:
        ...
        for dto in dtos:
            for order in getattr(dto, "working_orders", None) or []:
                # A TRAILING STOP'S TRIGGER IS THE VENUE'S, ALWAYS. Measured on a live instance book
    ...
    #:
    #: MEASURED 2026-08-26: the seed failed with `TimeoutError()` after 4m30s on alpaca-paper and 5m25s
    #: on ibkr-paper-retired, and `_publish_trades` served an EMPTY book for that whole window. The per-socket
    ...
    def _compute_book_truth(self, position_reports) -> None:
        ...
        # ibkr-paper, 2026-09-09, `book_truth: instruments 17, venue_unreadable 0` beside seven rows an
    ...
    async def _refresh_platform_probes(self) -> None:
        ...
        try:
            ...
            for sid in map(str, self._sibling_strategies):
                ...
                # production. An absent row is TRADING (2026-08-19), stated here, not hidden
    ...
    def _publish_account(self) -> None:
        ...
        if broker is not None:
            ...
            # alpaca-paper, the tenant that does most of the trading, never ran preflight at boot.
            # ibkr-paper-retired ran it only because nothing on an IBKR node publishes that topic.
            #
        ...
        if issuer in _TOTAL_IS_NET_LIQUIDATION:
            ...
            # includes it, so it double-counts the book (2026-08-24).
            #
            # WHATEVER CURRENCY THE ACCOUNT IS IN. The live IB paper account reports a single SGD leg
            # (`base_currency=None`, `total=1_000_016.15 SGD`) while every instrument is a USD equity, so
        ...
        # the unrealized PnL this would add, so it double-counts the book (2026-08-24).
        #
        ...
        # path — staging served equity 1,000,000.54 / cash 973,807.42 / lmv 0.00 over a held book,
...
def build_optional_strategy(name: str, builder):
    ...
    So the two are separated by TYPE, not by guesswork:
      ...
      RuntimeError      a REFUSAL: configuration, wiring, a missing universe, the wrong venue for the
                        lane. Until 2026-09-18 (#1123) this propagated, on the argument that a node
                        booting without a strategy its operator switched on is the failure the original
                        rule exists to prevent. It was measured twice as the WORSE failure: 2026-08-19
                        (QC345's universe fetch) and 2026-09-18, when `CRSI_ENABLED: true` on an Alpaca
                        instance — one builder's `RuntimeError("…requires SPLIT-ADJUSTED daily bars…")` —
                        would have crash-looped the engine and taken MANUAL/MOMENTUM/BCTROT/TECHIVOL/
                        QC345/SMHGLD down for one lane's config. Now recorded as `refused: …`, and the
                        node boots with every other lane.

      everything else   a BUG (TypeError, AttributeError, NameError). Propagates. A crash at deploy is
                        the right outcome for code that cannot run; recording it as "absent" would file
                        a broken build under the same heading as a stated refusal.

    LOUD, NOT SILENT. The skip is an ERROR log naming the strategy and the cause, AND a record
    (`skipped_builds()`) that `lanes_absent` carries to `/health` — subsystem `lanes` reads not-ok, the
    headline degrades, `subsystem_down:lanes` pages, `never_ran` unions the name, `make verify` reads
    it. "Degraded and saying so" is the outcome being bought here; "quietly absent" is still not
    acceptable, and a caller that wants to know can see the None. THREE KINDS on the record, because
    the operator's response differs: `unreachable:` means restart once the dependency is back;
    `refused:` means a restart changes nothing — fix the configuration.
    """
    try:
        strategy = builder()
        ...
        _SKIPPED_BUILDS[name] = f"unreachable: {type(exc).__name__}: {exc}"
        return None
    except RuntimeError as exc:
        _log.error(
            "%s REFUSED to build (%s: %s). The node is starting WITHOUT it; every other strategy is "
            "unaffected. A restart will not change this — the lane's configuration is wrong for this "
            "node. /health reads degraded until it is fixed or the lane is switched off.",
            name, type(exc).__name__, exc,
        )
        _SKIPPED_BUILDS[name] = f"refused: {type(exc).__name__}: {exc}"
        return None
...
def refuse_foreign_credentials(exec_provider: str, env: dict) -> None:
    ...
    asserted: the moment ibkr-paper-retired had an Alpaca key, the realized-P&L sweep polled an Alpaca
    ...
    for provider, names in _PROVIDER_CREDENTIALS.items():
        ...
        if present:
            raise RuntimeError(
                ...
                f"used — when ibkr-paper-retired last held one, the realized sweep polled an Alpaca "
...
def _boot_budget_or_refuse(*, connection: float, reconciliation: float, portfolio: float) -> float:
    ...
    # refused by name; the measured clean boot on ibkr-paper was 50.5 s.
    if connection < _CONNECTION_TIMEOUT_FLOOR_S:
        raise ValueError(
            ...
            f"(Nautilus's default; ibkr-paper's clean boot took 50.5 s) — a connect timeout shorter than the "
...
def build_node(cfg: FeedConfig | None = None) -> TradingNode:
    ...
    node_config_kwargs: dict = dict(
        ...
        #
        # ibkr-paper, 2026-09-11 07:18:13Z: STARTING, then sixty seconds later "Timed out (60.0s) waiting
    ...
    feed = UiFeedStrategy(
        ...
        # Which price series the provider's bars can be served in (#1124) — REQUIRED on the spec.
        price_adjustments=spec.price_adjustments,
    ...
    if exec_spec is not None:
        ...
        # is precisely the dead-exit state on ibkr-paper-retired this exists to end. The class default
    ...
    # MOMENTUM-001 (kumo-trading-strategies). Off unless KUMO_MOMENTUM_ENABLED is set; when it is on, the
    ...
    # SMHGLD-007 (issue 177, platform issue 953/#965) — the fixed-weight SMH/GLD sleeve. Registered
    ...
    # BCTROT-004 registers alongside MOMENTUM-002 (issue 32, the operator 2026-08-16). Registering
    # IS trading. An absent lifecycle row reads as TRADING (2026-08-19), so a lane that
    ...
    #
    # Registering IS trading: with no lifecycle row QC345 reads as TRADING (2026-08-19).
    ...
    # TECHIVOL-005 (issue 33, #63). Registered ONLY through `build_qc27_strategy`, which is
    ...
    # CRSISHORT-006 (issue 123, #858) — the SHORT book. Registered ONLY through
    ...
    # here is NOT trading for this lane — the gateway refuses TRADING until issue 131 (a
...
class _StopReenterWatch:
    ...
    NOT place or manage that stop itself, only detects when it fires (2026-08-02: "of course — every
...
# break + volume confirmation (2026-08-03: a new proxy metric, not full base/consolidation pattern
...
def _wilder_adx(bars: list, period: int = 9) -> dict:
    ...
    Wilder-correct and parity-verified against predecessor-repo's original pandas scanner per that file's own
...
_PYRAMID_ADX_PERIOD = 9  # the ledger's setting (ui/src/config/checklists.ts:16), not the textbook 14
...
class _PeakWatch:
    ...
    2026-08-03: a resting-stop replace is free, but each partial trim crosses the spread for real).

    KNOWN LIMITATION (codex review round 2, disclosed not silently missed; 2026-08-03: reorder trail-
...
class _PyramidWatch:
    ...
    gates the exit — 2026-08-03: manual, no auto sector mapping exists), `add_r_multiple` (fraction of
