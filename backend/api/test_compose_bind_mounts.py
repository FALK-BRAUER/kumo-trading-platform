...
WHAT HAPPENED (kumo-trading-platform issue 524). `compose.paper.yml` mounted ledger-tool's rotation tooling as

    - ../../ledger-tool/tools:/ledger-tool/tools:ro

Relative paths in compose resolve from the COMPOSE FILE'S directory. Run from the platform repo that
is `~/projects/ledger-tool/tools` and correct. But `kumo-trading-platform/instances` stages each pinned
ref into `.stage/<instance>/cockpit/`, so the same line resolves to
`.stage/<instance>/ledger-tool/tools` — which does not exist, so Docker CREATES IT EMPTY and mounts that.

Measured 2026-08-24 on the running engine:

    docker inspect ... /ledger-tool/tools ->
        /host_mnt~/projects/kumo-trading-platform/instances/.stage/alpaca-paper/ledger-tool/tools
    contents: 0 files
...
`additional_contexts: strategies: ../../kumo-trading-strategies`, which failed loudly with
`failed to get build context strategies: stat .../kumo-trading-strategies: no such file`. That one failed
...
def test_no_compose_bind_mount_uses_a_RELATIVE_host_path(path: Path):
    ...
    assert not offenders, (
        f"{path.name} has relative host bind mounts. Staged from kumo-trading-platform/instances these "
