...
So the boot gate ran only where there was no broker account frame. test-alpaca, the tenant that does
most of the trading, has never run it. staging-ibkr ran it and reported DEGRADED twice, and those
...
def _alpaca_frame() -> dict:
    ...
    non-None on test-alpaca and None on staging-ibkr."""
    return {
...
def test_the_boot_gate_RUNS_when_a_broker_account_frame_exists():
    ...
    Without the fix this list is empty: the gate has never run on test-alpaca, for its whole life.
    """
...
def test_the_boot_gate_STILL_runs_on_a_node_with_no_broker_frame():
    """The other direction — staging-ibkr, where the gate already worked. Hoisting the call must not
