"""QC345 activation readiness (#318/#320, issue 34 and #37).
...
#: The promoted live-feasible candidate (kumo-trading-strategies PR #36). NOT the strategy's own defaults —
...
def test_qc345_is_registered_with_the_right_identity():
    ...
    It was briefly 003, assigned by taking the next free number without checking what kumo-trading-strategies
    had already named. issue 32 is titled "BCTROT-003", so 003 was spoken for — and two
...
def test_the_schema_blocker_is_RESOLVED_and_the_domain_is_generated():
    """issue 37, fixed upstream in 261710d — and this test is the record of how it was found.
...
def _unused_the_adapter_has_LANDED_and_carries_the_identity_cockpit_allocated():
    """issue 34 is resolved — the blocker test fired and named this follow-up.
    ...
    not: I had reserved 003 for BCTROT on the strength of issue 32's title and moved QC345
...
def test_BOTH_shipped_adapters_satisfy_the_registration_contract():
    """issue 39 implemented (33eee4c). The third blocker test to fire and be retired today.
