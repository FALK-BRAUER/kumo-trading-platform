"""Consumer for kumo-strategies' claims invariant (#437, kumo-strategies#66).

THE PREDICATE IS THEIRS AND IS NOT REIMPLEMENTED HERE. `over_claimed` landed on kumo-strategies main
...
class PredicateAbsent(RuntimeError):
    """The installed kumo-strategies pin does not carry the predicates (#903). Named, so the two
...
def _predicates():
    ...
    try:
        ...
        raise PredicateAbsent(
            ...
            f"on this kumo-strategies pin — the claims invariant CANNOT be computed: {exc!r}"
        ) from exc
