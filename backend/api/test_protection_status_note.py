...
--------------------
On 2026-08-19 a PEAK arm on LFST failed and the UI said, in red:
...
A `PROT-SELL-LFST` covering the full 429 shares was resting at the broker the entire time. The message
...
def _note(covered, *, cleared_count=2, reason="the venue did not confirm the cancel"):
    ...
    return asyncio.run(
        UiFeedStrategy._protection_status_note(
            host, reason, cleared_count, "LFST.XNAS", "MANUAL-001", "LONG"
        )
    )


def test_a_still_protected_position_is_not_called_unprotected():
    """The LFST case, verbatim. 429 shares were covered while the operator was told to check the broker."""
