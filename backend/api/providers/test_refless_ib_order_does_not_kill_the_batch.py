...
MEASURED ON ibkr-paper, 2026-09-02T16:22:53Z -> 2026-09-03T14:53:58Z (22h): IB account DUPTEST02 held
...
#: Eighteen venue ids, the count ibkr-paper actually choked on over the 22h log. The SHAPE of the batch
...
def test_the_fixture_can_express_the_bug__an_UNFILTERED_batch_dies_on_ONE_refless_order():
    ...
    ibkr-paper's `venue_reported_ids` was empty rather than short by 18.
    """
...
def test_the_REAL_ibkr_paper_book_reports_every_one_of_its_21_live_orders():
...
def test_the_COCKPIT_read_that_FEEDS_PROTECTION_comes_back_with_reports_not_None():
    ...
    because half a broker read is worse than none — 1,217 refusals on ibkr-paper in 22h. That refusal
    ...
    # ibkr-paper refuse to place protection. Without this the assertion below could not fail.
...
def test_the_NATIVE_config_knob_does_NOT_avoid_this__measured_not_assumed():
    ...
    It DEFAULTS TO FALSE and `api/providers/ibkr.py` does not set it, so ibkr-paper was already on the
...
def test_the_change_line_is_an_ERROR_and_fires_ONLY_when_the_ROWS_change():
    ...
    Cadence: this runs every ~6s on ibkr-paper. An ERROR per offender per tick is 18 lines every six
