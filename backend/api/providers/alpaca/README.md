...
- `shortable.py` — the borrow plane (#1102 d): `feed.shortable_provider` from the `/v2/assets` table (tradable + shortable + easy_to_borrow → LOCATABLE, else None by name), one load per 26 h, the IB plane's verbs and health words.
