...
def _unrealized_totals(positions: list[dict]) -> dict:
    ...
    for key, field in (
        ...
        for pos in positions or []:
            ...
            # halt in kumo-strategies 43c6d3e.
...
#
# Measured on the live paper account, `period=1D`, 2026-08-25 09:45 ET:
#
...
class AlpacaExecutionClient(LiveExecutionClient):
    ...
    async def _report_reconcile_drift(self) -> None:
        ...
        for raw in broker_positions:
            ...
            cockpit_qty = cache_qty.get(sym, Decimal(0))
            if broker_qty != cockpit_qty:
                drift.append({"symbol": sym, "broker_qty": float(broker_qty), "cockpit_qty": float(cockpit_qty)})
        # A symbol the cockpit thinks it holds but the broker does not (phantom) is drift too.
        for sym, cockpit_qty in cache_qty.items():
            if sym not in seen and cockpit_qty != 0:
                drift.append({"symbol": sym, "broker_qty": 0.0, "cockpit_qty": float(cockpit_qty)})
    ...
    async def _report_account_state(self) -> None:
        ...
        for key in ("portfolio_value", "equity"):
            ...
            # kumo-strategies 43c6d3e had `broker_equity` return NaN, where `nan <= 0` is False so
