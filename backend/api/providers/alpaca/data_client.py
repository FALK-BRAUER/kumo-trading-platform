...
from api.providers.alpaca.http import PRICE_ADJUSTMENTS, AlpacaHttpClient
from api.providers.alpaca.shortable import build_plane as _build_shortable_plane
...
class AlpacaDataClient(LiveMarketDataClient):
    ...
    async def _connect(self) -> None:
        ...
        # — a request a lane cannot issue for a symbol it failed to resolve. Measured on an Alpaca paper instance
    ...
    async def _request_bars(self, request: RequestBars) -> None:
        ...
        # THE ADJUSTMENT RIDES THE REQUEST (#1124). Raw unless the requester names `split`; the
        # value is validated at the HTTP door, so `all` is refused by name here rather than sent.
        # STAMPED back onto the response params below — the requester reads the fact, never
        # re-derives it from what it asked for. Nautilus hands `params` over as `{}` when absent.
        adjustment = str((request.params or {}).get("adjustment") or "raw")
        if adjustment not in PRICE_ADJUSTMENTS:
            # Refused BY NAME before the door, never degraded to raw: a lane that asked for adjusted
            # bars and silently got raw ones is #854 with a green log line. Same constant as the door.
            raise ValueError(
                f"{bar_type}: adjustment {adjustment!r} is not a series this client serves "
                f"({sorted(PRICE_ADJUSTMENTS)}) — #1124")
        params = {**(request.params or {}), "adjustment": adjustment}
        ...
        self._handle_bars(bar_type, bars, request.id, request.start, request.end, params)
...
def build_data(provider_config: dict) -> DataClientSpec:
    ...
    return DataClientSpec(
        ...
        # Both series, PER REQUEST (#1124): `adjustment=raw` by default, `split` for the requester
        # that names it. Measured 2026-09-18: NVDA 2024-06-07 120.89 split / 1208.88 raw.
        price_adjustments=PRICE_ADJUSTMENTS,
        # SHORTABILITY from the asset table (#1102 d): declared HERE, inside the connector, exactly
        # as IB declares its tick-236 plane (ibkr.py), so the engine attaches it with no venue
        # import. The plane gets its OWN HTTP client — the data client's is built inside the
        # Nautilus factory after this spec exists.
        shortable_plane=lambda publish, now_ns: _build_shortable_plane(
            AlpacaHttpClient(key=api_key, secret=api_secret,
                             trading_base=config.trading_base_url, data_base=config.data_base_url),
            now_ns),
