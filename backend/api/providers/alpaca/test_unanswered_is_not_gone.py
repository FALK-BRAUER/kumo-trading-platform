...
def _instrument():
    """Read at CALL time, not import time: in the public tree the capture is absent and the module
    must still IMPORT so the `captured_fixture` skip can name why its tests do not run (#1045). A
    module-level read raised FileNotFoundError during collection, which reads as a broken test."""
    return cr.to_obj(cr.CacheImage.from_fixture(FIXTURES / "redis.json").instruments["PATH.XNYS"])
