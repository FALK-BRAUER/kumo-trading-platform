...
in the protocol kumo-trading-strategies defined (7bf2a2c): `LOCATABLE` (identity) for "available, fee
...
Measured on ibkr-paper 2026-09-10 12:50Z (own client id, read-only): 118 of the backtest's 130 names
...
class ShortableRegistry:
    ...
    def __init__(self, *, publish: Callable[[DataType, Data], None]) -> None:
        ...
        #: Whether the installed kumo-trading-strategies carries the LOCATABLE contract the provider speaks.
        #: THREE STATES: "unknown" until `borrow_rates_provider` has looked, then "present" or
        #: "absent". ibkr-paper's pin 0fbcfff predates the module; a build-time import here crashed
...
def borrow_rates_provider(registry: ShortableRegistry, *, now_ns: Callable[[], int],
    ...
    """`symbols -> {symbol: LOCATABLE | None}` in kumo-trading-strategies' protocol. LOCATABLE is identity
