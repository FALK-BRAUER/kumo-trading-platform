...
Measured on an IBKR paper instance 2026-08-29: the pool holds 106 symbols, `feed.toml`'s `[universe]` holds 93 —
...
def test_an_EXPLICITLY_EXCLUDED_symbol_is_not_reintroduced_by_the_pool(monkeypatch):
    ...
    cannot express "absent on purpose". Measured on an IBKR paper instance by running this branch's own code:
...
def test_a_LOWERCASE_pool_row_cannot_evade_the_exclusion(monkeypatch):
    ...
    refused (#663) — while `ledger.py` writes `str(r.symbol)` straight from the same unvalidated CSV
...
def test_an_ENABLED_lanes_settings_universe_is_in_the_tradeable_set(monkeypatch):
    """ibkr-paper 2026-09-10: `QC27_UNIVERSE` 138 names, TECHIVOL-005 booted "125 of 138 symbols have
...
def test_an_ENABLED_fixed_symbol_lane_is_in_the_tradeable_set_even_without_an_explicit_override(monkeypatch):
    """ibkr-paper 2026-09-14: SMHGLD-007 decided, GLD filled, SMH died inside IBKR submit with
    AttributeError('NoneType' object has no attribute 'is_inverse'). The lane had its gate and
    allocation on, but no explicit `SMHGLD_SYMBOLS` override because the schema default is the two
    legs. The strategy builder saw that default; the IBKR load_contracts union did not."""
    monkeypatch.setattr(ibkr, "_pool_symbols", lambda: _POOL)
    monkeypatch.setattr(ibkr, "_declared_symbols", lambda: _SNAPSHOT)
    monkeypatch.setattr("api.settings.resolve", lambda _d: {"SMHGLD_ENABLED": True})
    out = set(ibkr._tradeable_symbols())
    assert {"SMH", "GLD"} <= out, (
        "an enabled fixed-symbol lane never reaches IBKR load_contracts, so the first entry into a "
        "missing leg fails at submit with a None instrument"
    )
    assert set(_SNAPSHOT) | set(_POOL) <= out, "the fixed-lane union must not drop what it had"
    data = {c.symbol for c in ibkr._data_contracts()}
    assert {"SMH", "GLD"} <= data, "the DATA client must resolve fixed lane legs too, or no bars arrive"


def test_a_DISABLED_fixed_symbol_lane_is_NOT_loaded(monkeypatch):
    """Fixed legs still cost IBKR contract resolution. A disabled fixed-symbol lane must not expand
    the load set just because the platform knows its defaults."""
    monkeypatch.setattr(ibkr, "_pool_symbols", lambda: _POOL)
    monkeypatch.setattr(ibkr, "_declared_symbols", lambda: _SNAPSHOT)
    monkeypatch.setattr("api.settings.resolve", lambda _d: {"SMHGLD_ENABLED": False})
    assert not {"SMH", "GLD"} & set(ibkr._tradeable_symbols())


def test_fixed_symbol_lane_defaults_match_the_settings_shape():
    """The connector table has to mirror the lane's own empty/absent-symbols rule, because live
    settings files usually omit defaulted values."""
    from api.lane_universes import enabled_lane_universe_symbols

    implicit = enabled_lane_universe_symbols({"SMHGLD_ENABLED": True})
    empty_override = enabled_lane_universe_symbols({"SMHGLD_ENABLED": True, "SMHGLD_SYMBOLS": []})
    disabled = enabled_lane_universe_symbols({"SMHGLD_ENABLED": False})
    assert set(implicit["SMHGLD_SYMBOLS"]) == {"SMH", "GLD"}
    assert set(empty_override["SMHGLD_SYMBOLS"]) == {"SMH", "GLD"}
    assert "SMHGLD_SYMBOLS" not in disabled
...
def test_EVERY_lane_universe_in_the_table_is_read_and_the_lanes_read_the_same_table():
    ...
    if not crsishort_installed():
        pytest.skip("CRSISHORT adapter absent from the installed kumo-trading-strategies (loud test is red)")
