...
#: strategies.
#:
#: Measured on staging 2026-08-29: connect used 17.6s of 60s — 41s of headroom — while five bad
...
def build_data(provider_config: dict) -> DataClientSpec:
    ...
    return DataClientSpec(
        ...
        config=InteractiveBrokersDataClientConfig(
            ...
            # SILENTLY — no error code, nothing. Measured on staging2 across the whole 2026-09-09
        ...
        # through `reqTickByTickData` too, so it is refused the same way — staging2's boot of
...
def build(exec_config: dict) -> ExecClientSpec:
    ...
    config = InteractiveBrokersExecClientConfig(
        ...
        # RECONCILIATION brings in with open positions. On 2026-08-24 staging-ibkr had qualified
...
def _reference_symbols() -> tuple[str, ...]:
    ...
    found". Measured on staging 2026-08-27: 0 of 27 compass tickers were in the tradeable universe,
...
def _tradeable_symbols() -> tuple[str, ...]:
    ...
    # and on an instance with no catalog nothing ever IS refused (#663) — while george.py writes
...
def _lane_universe_symbols() -> tuple[str, ...]:
    ...
    Measured on staging2 2026-09-10: TECHIVOL-005 "125 of 138 symbols have no instrument on this
...
def _pool_symbols() -> tuple[str, ...]:
    ...
    does not refresh with the pool". Measured on staging 2026-08-29: pool 106, snapshot 93, and
