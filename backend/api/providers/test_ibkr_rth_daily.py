...
measured on RTH daily bars; today it registers on ibkr-paper only under a STATED deviation
...
class _Recorder:
    ...
    # The bar CONVERSION the volume-floor wrap targets (#1084), with the installed signature's
    # parameter names — the two wraps share one client instance and one factory.
    async def _ib_bar_to_nautilus_bar(self, bar_type, bar, ts_init, is_revision=False):
        self.calls.append(("_ib_bar_to_nautilus_bar", {"bar_type": bar_type, "volume": bar.volume}))
        return None
...
def test_the_REPO_FACTORY_returns_a_data_client_whose_IB_client_carries_the_wrap(monkeypatch):
    ...
    assert mod.is_volume_floor_installed(out._client)      # #1084 rides the same factory
