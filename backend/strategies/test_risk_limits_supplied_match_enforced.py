...
`pgrunner.py:517` records the same class happening once before, in kumo-trading-strategies' own words:
...
EXPECTED = {
    # qc27_runner now reads TWO. `daily_loss_frac` was added by kumo-trading-strategies ea21f98/dfc8464
    ...
    #
    # WHY IT IS SET AT ALL. 2026-09-08: "It is 1.0. In reality it is lower because of math,
    ...
    # kumo-trading-strategies' own momentum_rotation/config.py:117 records that `_submit` has no aggregate
...
def _reads(module_path: str) -> set[str]:
    ...
    src = inspect.getsource(real_module(module_path))
...
def test_QC345s_notional_cap_CANNOT_BIND_at_todays_allocation():
    ...
    Suggested by the kumo-trading-strategies peer, whose own `test_at_STAGING_SIZE...` does this for the
