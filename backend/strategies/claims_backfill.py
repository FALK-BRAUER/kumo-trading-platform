...
today's price asserts a peak that never happened (kumo-trading-platform issue 197 B1).
...
async def _record_claim(journal, strategy_id: str, symbol: str, qty, entry_px: float, *,
    ...
    THE DELETE-RACE (kumo-trading-platform issue 845, codex scope review). The sweep reads "held 6, no claim" and
...
async def backfill_claims(broker, journal, strategy_id: str) -> dict:
    ...
    for symbol, qty in sorted(held.items()):
        if symbol in claimed:
            ...
            # marker does not exist yet (the claims row is kumo-trading-strategies' schema) and inventing
        ...
        try:
            ...
            # issue 124) and a Postgres error can land on any write; either way this
