"""CRSISHORT-006 SENDS ORDERS (cockpit#858). The seam, driven: gateway.run() -> the venue.
...
def _run(gw, *, order_path=True, slot=SLOT):
    """`order_path` forces kumo-strategies' ORDER_PATH_COMPLETE for the duration.
...
def test_a_PRE_OPEN_slot_in_SETTINGS_reaches_the_resolver_the_builder_passes(monkeypatch):
    ...
    kumo-strategies refuses at construction when a trading lane's slots cannot reach the auction and
