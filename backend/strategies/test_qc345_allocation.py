...
1. SIZING OFF THE WHOLE ACCOUNT. `_equity_per_position` reads `self._broker.equity()`, which is the
   ACCOUNT's ~103,466, and QC345's allocation is 20,000. Every other runner in kumo-strategies reads
...
def test_a_missing_allocated_equity_FIELD_does_not_kill_the_session() -> None:
    """This repo pins kumo-strategies by revision. An unguarded `replace()` on a RiskLimits without
