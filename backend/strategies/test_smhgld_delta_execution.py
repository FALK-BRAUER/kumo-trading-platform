"""SMHGLD delta execution (kumo-cockpit#953): a TARGET and a DELTA, executed; never `enter`/`exit`.

The lane's contract is `order_plan()` (kumo-strategies f36b674): a tuple of `Order(symbol, delta,
...
Five things the gateway must NOT re-derive (coordinator, from l21's code): the ordering, the
rounding, the target, the delta, and the claim's value — which is `post_trade_qty`, NEVER
`target_qty` (they differ by up to one lot on every resize, and nothing in the system would ever
disagree with a claim written from the unrounded target: the broker holds the rounded number and
`over_claimed` treats a sub-lot under-claim as safe).
...
def test_the_claim_is_set_ABSOLUTELY_to_post_trade_qty_NEVER_target_qty():
    """Lab's find. held 130 − 12 = 118, not the unrounded 118.4; held 214 + 9 = 223, not 223.6."""
    journal, broker = _Journal(), _Broker()
    gw = _gateway("TRADING", journal, broker, _plan(SELL_SMH, BUY_GLD))

    _run(gw)

    assert gw.write_claim.calls == [("SMH", 118.0, 240.00), ("GLD", 223.0, 310.00)]
    assert all(q != o.target_qty for (_s, q, _p), o in zip(gw.write_claim.calls, (SELL_SMH, BUY_GLD)))
...
def test_no_claim_writer_wired_refuses_the_WHOLE_session_not_just_the_claim():
    """An order accepted with no claim written is an unclaimed position every neighbour can reach
    into (#941's rule, same here). The writer is a precondition of sending, not a follow-up."""
    journal, broker = _Journal(), _Broker()
...
def test_the_INSTALLED_kumo_strategies_carries_the_seam_this_gateway_needs():
    ...
    kumo-strategies 06f3055. Until then the two tests below were `xfail(not installed, strict=True)`
...
def test_an_ALL_BUY_opening_plan_executes_both_legs_from_zero_and_files_no_incident():
    ...
    that matters most. Both buys go, claims are set to each post_trade_qty, no `no_position` incident
    (the plan opens both), the notionals split ~32/68 of the SLEEVE."""
    ...
    assert gw.write_claim.calls == [("SMH", 131.0, 240.0), ("GLD", 216.0, 310.0)]
    assert journal.coded("no_position", kind="incident") == []
