...
`momentum.py` wires MOMENTUM-001 from the `kumo-trading-strategies` package into the node. The strategy class
...
reads as TRADING (2026-08-19), so there is no second operator approval between the deploy and
...
`QC345ComputedSource` does every selection stage. The preservation floors from issue 43
