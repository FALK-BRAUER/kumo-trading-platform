...
(kumo-strategies#122) call `runner.sync_claim(sym, lane_qty, px)` from every terminal handler; the
...
def _dml(executed):
    """The ledger statements minus the key lock `store.write_claim` takes first (kumo-strategies#124):
