...
WHY THIS FILE EXISTS. The maths used to live in `ledger-tool/tools/{rotation_read,grade_full}.py`,
reached at runtime by `sys.path.insert(0, "/ledger-tool/tools")` against a read-only Docker mount, behind
`KUMO_LEDGER_TOOL_TOOLS` and a `LedgerToolUnavailable` exception. The feature was always kumo's — the data
...
ibkr-paper-retired showed "No rotations to show", and why `KUMO_LEDGER_TOOL_TOOLS=/nonexistent` read as a
