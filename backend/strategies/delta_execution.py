"""Delta execution — the lane-AGNOSTIC half of executing a target-and-delta plan (kumo-trading-platform issue 953).

Takes `tuple[Order, ...]` (symbol, delta, target_qty, held_qty, reason, post_trade_qty), a submit
seam, a sleeve, prices and a journal-row writer — and knows NOTHING about which lane produced the
plan. SMHGLD is its first consumer (`strategies/smhgld.py`); issue 184
...
WHAT IT GUARANTEES, per plan:
  ...
  * buys are budget-gated (`api.budget_gate.may_submit`), sells never are — reducing exposure needs
    ...
    unknown=allowed);
  * on acceptance NO claim is written: submit acceptance is local, while the claim follows the
    terminal order event and the lane's actual Nautilus cache quantity (`runner.sync_claim`);
...
async def execute_plan(orders, *, broker, write_claim, row: RowWriter, sleeve, deployed: float,
    ...
    for o in orders:
        ...
        code = await _send(o, side, qty, px, session=session, slot=slot, strategy_id=strategy_id,
                           broker=broker, row=row, sent=sent,
...
async def _send(o, side: str, qty: int, px, *, session: str, slot: str, strategy_id: str, broker,
                row: RowWriter, sent: list, limit_px: float | None = None,
