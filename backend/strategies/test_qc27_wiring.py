"""TECHIVOL-005 registers ONLY through the safe path (kumo-strategies#33, #63).
...
kumo-strategies' `tests/runtime/executor/test_qc27_runner.py`, which is the repo that defines those
...
def test_NO_construction_of_the_adapter_anywhere_omits_the_session_runner():
    ...
    offenders = []
    # OUR SOURCE ONLY. `_BACKEND.rglob` also walks `.venv/`, where kumo-strategies' own
...
def test_an_ABSENT_lifecycle_row_is_TRADING():
    """Operator, 2026-08-19, and deliberately NOT re-derived here — it mirrors
...
#: Config fields whose values were MEASURED and whose reasoning lives in kumo-strategies'
...
def test_the_decision_slot_is_DERIVED_from_the_strategys_own_offset():
    """A literal here is a second derivation of a fact kumo-strategies owns — and the two drifting is
...
def test_the_builder_CONSTRUCTS_a_real_strategy_end_to_end(monkeypatch, tmp_path):
    ...
    # A VALUE THE FALLBACK COULD NOT PRODUCE (codex, 2026-08-25). This seeded nothing for the lane's
...
#   absent  -> None, and the runner falls back to the ACCOUNT (kumo-strategies be244d9 confirms it
...
def test_an_ABSENT_allocation_is_STOPPED_HERE_and_never_becomes_the_whole_account(
    ...
    I FIRST WROTE THIS ASSERTING `is None`, and it was wrong. The reasoning came from kumo-strategies'
    ...
    # The account on test-alpaca is ~103k against a 20k lane. Pin the GAP, not just the value: this
...
def test_NO_lane_cockpit_builds_collapses_an_ABSENT_allocation_into_a_ZERO():
    ...
    express a zero at all. kumo-strategies had the same `or` at two sites (be244d9). This is the third
