...
WHY THIS EXISTS SEPARATELY. kumo-strategies db25413 lets an adapter re-read its decision slot on every
...
def _live_reread_kwargs(strategy_cls, **readers) -> dict:
    ...
    THIS REPO PINS kumo-strategies BY REVISION, and the seam arrived in db25413. Passing
    ...
    if missing:
        _log.warning(
            "%s: installed kumo-strategies does not accept %s — the decision slot stays captured at "
