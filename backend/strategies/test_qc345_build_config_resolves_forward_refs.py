...
Measured on an Alpaca paper instance 2026-09-11 06:53Z, first market-aware poll after deploying cockpit f05979c /
...
def test_FIXTURE_the_installed_annotation_for_market_view_is_a_forward_ref_STRING():
    ...
    if field is None:
        pytest.skip("installed kumo-trading-strategies QC345RotationConfig declares no market_view (a pin before e600383)")
...
def test_a_forward_referenced_nested_group_is_built_as_its_dataclass_not_passed_through_as_a_dict():
    ...
    if "market_view" not in {f.name for f in dataclasses.fields(cls)}:
        pytest.skip("installed kumo-trading-strategies QC345RotationConfig declares no market_view")
...
def _old_build_config():
    ...
    from api.test_not_shipped import require_history
    require_history(_PRE_FIX_REVISION)
