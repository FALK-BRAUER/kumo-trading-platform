...
silently absent", and its sibling "a lane that is switched on and silently ARMED":
  ...
  * the gateway in TRADING REFUSES with a `risk` row while the installed order path cannot carry a
    resting short LIMIT (kumo-strategies#131) — an operator flipping the row early gets a loud
  ...
  * split-adjusted bars are a construction-time requirement of the adapter, satisfied on IBKR
    (measured: NVDA 2024-06-07 = 120.89 from staging2's gateway) and NOT on the raw Alpaca path.
"""
...
pytestmark = pytest.mark.skipif(
    ...
    reason="installed kumo-strategies has no CRSISHORT adapter — see "
...
def _settings(monkeypatch, **over):
    base = {"CRSI_ENABLED": True, "CRSI_UNIVERSE": ["IONQ", "RGTI", "SMCI", "MSTR"],
            ...
            # kumo-strategies refuses at construction when a trading lane's slots cannot reach the
...
def test_the_builder_REFUSES_raw_bars_on_a_non_IBKR_feed(monkeypatch):
    """ADJUSTED is a fact about where the bars come from. Cockpit's Alpaca path is `adjustment=raw`
    with a tripwire (`providers/alpaca/test_http.py`), so on that feed the honest answer is a
    refusal naming #854 — never a strategy constructed on prices that book a reverse split as a
    -1000% short loss."""
    from strategies import crsi_short

    _stub_store_and_calendar(monkeypatch)
    _settings(monkeypatch, CRSI_BORROW_GATE_OFF=True)
    with pytest.raises(RuntimeError, match="854"):
...
def test_SHADOW_journals_the_full_intent_and_submits_NOTHING():
    ...
    # `session_journal()` — which finds it as the PUBLIC attribute `journal` (kumo-cockpit#587).
...
def test_TRADING_is_REFUSED_with_a_risk_row_while_the_limit_path_is_absent():
    ...
    on kumo-strategies#131). TRADING therefore refuses LOUDLY — a `risk` row naming #858 and the
    ...
    # RESPELT, NOT LOOSENED (kumo-strategies#131). "no submission path" became FALSE: the gateway
...
def test_registration_WRITES_the_SHADOW_row_when_none_exists_and_keeps_an_explicit_one(monkeypatch):
    """"Registered in SHADOW" is a row, not a hope: an absent row reads TRADING (Operator, 2026-08-19).
...
def test_the_builder_DECLARES_shadow_only_where_the_adapter_asks(monkeypatch):
    ...
    cockpit#853's first phase and the safest way to validate a lane against a venue.

    The flag was also DEAD upstream until 2425465 — assigned at `crsi_short.py:376`, read nowhere,
    gating only a build-time raise (cockpit#1027). Asserting it is PASSED is therefore not enough
    ...
    assert passed.get("shadow_only") is True, (
        ...
        "exact defect cockpit#1027 reported one level up")
