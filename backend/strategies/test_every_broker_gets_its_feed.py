...
own `NautilusBroker` and never assigned it, so the moment kumo-strategies routed their exits through
...
named two strategies of four, and kumo-strategies' discovery floor named three of five. Both passed
while a live strategy was broken. So this scans the directory: a gateway added tomorrow is covered
without anyone remembering.

kumo-cockpit#459, second half. The routing half is kumo-strategies 53e2ff5.
"""
...
def _builder_names() -> set[str]:
    ...
    A SECOND INDEPENDENT DERIVATION, not a list. Borrowed from kumo-strategies, which hit the same
...
def test_the_engine_ACTUALLY_PASSES_a_feed_to_every_builder():
    ...
    error as `test_broker_equity_seam`'s hardcoded list and kumo-strategies' three-name floor, in the
