...
def test_an_ABSENT_lifecycle_row_is_TRADING(monkeypatch):
    """An absent row means TRADING (Operator, 2026-08-19). INVERTED from the original rule; read why.
    ...
    # default stays DISABLED — that constructor is kumo-strategies' and is not ours to redefine.
    assert Lifecycle().state is State.DISABLED, "kumo-strategies' own default changed under us"
...
def test_EVERY_gateway_accepts_the_SessionRunner_protocol_signature():
    ...
    kumo-strategies already wrote this contract down after the SAME failure on 2026-08-17
...
def test_a_back_fill_whose_WRITE_FAILED_journals_a_RISK_row_not_silence(monkeypatch):
    """`store.write_claim` can stall (kumo-strategies f54f54d, `ClaimWriteStalled`) and the sweep
