...
THE MEASURED FAILURE. On ibkr-paper-retired — an instance with *"NO business with alpaca whatsoever"*
...
def test_an_UNCLASSIFIABLE_instrument_is_NAMED_at_ERROR_and_never_silently_ranked(caplog):
    ...
    # Missing name means EXCLUDE (kumo-trading-strategies 4d46290): the row travels with name=None so the
...
def test_the_ledger_status_travels_into_the_frame_so_terminal_buckets_can_fire():
    """`terminal_buckets` (kumo-trading-strategies engine.py) reads the `status` column by NAME and answers
