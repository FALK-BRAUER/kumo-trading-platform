...
WHAT THIS MODULE IS. The cockpit half of issue 123: the builder that constructs the
...
WHAT IS DIFFERENT FROM THE LONG LANES, each a thing that was assumed everywhere else:
  ...
  prices     The adapter REFUSES to construct on anything but split-adjusted bars (a reverse split
             ...
             where the bars come from, so it is decided here from the feed's DECLARATION
             (`DataClientSpec.price_adjustments`, #1124), never from the provider's name: IB DAY
             bars are `whatToShow=TRADES`, split-adjusted always (ibkr-paper's gateway, NVDA
             2024-06-07 close 120.89, the post-10:1 figure); Alpaca serves `split` PER REQUEST and
             gives the same 120.89 (raw: 1208.88; measured 2026-09-18). A feed whose set lacks
             `split`, or that never declared one, is refused by name.
  ...
  orders     The installed `broker.py` submits MARKET DAY orders and nothing else
             (issue 131). CRSISHORT's entry is a sell-short LIMIT resting one session and
...
absent row reads as TRADING (2026-08-19) — which is why the deploy that enables this lane
...
#: recorded on issue 131 and not built here. The installed broker CAN carry a LIMIT and a
#: cancel since kumo-trading-strategies 37569de; that is not the gate. Named in every refusal.
_NO_ORDER_PATH = ("CRSISHORT will not submit (platform issue 858): kumo-trading-strategies ORDER_PATH_COMPLETE is "
...
def _requested_adjustment() -> str | None:
    """The price series the installed CRSISHORT adapter ASKS the data client for, off its own
    `HISTORY_REQUEST_PARAMS` (ks#262) — None when the installed pin has no such request."""
    try:
        from kumo_strategies.runtime.nautilus.crsi_short import HISTORY_REQUEST_PARAMS
    except ImportError:
        return None
    value = HISTORY_REQUEST_PARAMS.get("adjustment")
    return str(value).strip().lower() if value else None


def _price_adjustments(feed) -> frozenset[str] | None:
    """Which price series the feed's historical bars can be served in — the provider's own
    declaration (`DataClientSpec.price_adjustments`, #1124), read off the feed. None = undeclared,
    which is its own answer, never read as raw."""
    declared = getattr(feed, "_price_adjustments", None)
    return frozenset(str(x).strip().lower() for x in declared) if declared else None
...
#: (2026-08-19); for a lane whose gateway refuses TRADING that is safe but it is not what
...
class CrsiShortSessionGateway:
    ...
    looks for it there to write the session's `state` row (kumo-trading-platform issue 587 — a gateway that
    ...
    def _can_execute(self) -> bool:
        ...
        `ORDER_PATH_COMPLETE` IS THE AUTHORITY, READ LIVE FROM kumo-trading-strategies — never mirrored here.
    ...
    async def _submit(self, intent, session: str, state, _result):
        ...
        # 77.6% of the return comes from (kumo-trading-strategies `crsi_short.py:89`). Reading the phase
        ...
        # return without failing anywhere (kumo-trading-strategies `crsi_short.py:115`).
...
class _ShortOrder:
    ...
    `CLAIMS_REPRESENT_SHORT` (issue 173) means. Pinned against the seam's real contract by
...
async def _ensure_registration_row(sm) -> str:
    ...
    async with sm() as s:
        ...
        s.add(StrategyState(
            ...
            reason="registration: CRSISHORT-006 starts in SHADOW (platform issue 858) — computes and "
...
def build_crsi_short_strategy(*, feed=None):
    ...
    # predate issue 121 (2026-09-11: not now); with the gate ON and no adapter the
    ...
    try:
        ...
        raise RuntimeError(
            f"strategies.CRSI_ENABLED is on but the installed kumo-trading-strategies has no CRSISHORT "
            f"adapter ({exc}) — the pin predates issue 121. Unset the gate or move the "
            f"pin; the lane cannot register on this revision.") from exc

    # BARS: split-adjusted AND regular-trading-hours, both decided by where they come from — and
    # both read off the provider's DECLARATION, never its name (#1124, 2026-09-18). The lane was
    # measured on split-adjusted bars (NVDA 2024-06-07 = 120.89: IB's TRADES bar, and Alpaca's
    # `adjustment=split` bar, to the cent; Alpaca's raw bar is 1208.88). Until #1124 this checked
    # `provider != "ibkr"` and refused Alpaca for its name — the refusal that would have crash-looped
    # paper under `CRSI_ENABLED: true` (#1102, #1123) for a series Alpaca serves. Three states: the
    # declaration names `split` (the adapter's daily request asks for it — ks half of #1124, with
    # `disable_historical_cache` so the split series stays out of the shared cache); it does not
    # (refused, naming the set); it was never made (refused as undeclared, never read as raw).
    # THE TWO HALVES MEET HERE: what the installed ADAPTER ASKS FOR (ks#262 `HISTORY_REQUEST_PARAMS`,
    # the dict that rides `request.params` into the data client) against what the DATA CLIENT
    # DECLARES it can serve. Read off the adapter, never retyped — on a pin that predates ks#262
    # the adapter asks for nothing and gets RAW bars on Alpaca whatever the door offers, so an
    # adapter that cannot ask is refused as such (ks review of #1128).
    requested = _requested_adjustment()
    if requested is None:
        raise RuntimeError(
            f"strategies.CRSI_ENABLED is on but the installed kumo-trading-strategies adapter requests no "
            f"price adjustment (`HISTORY_REQUEST_PARAMS` absent — the pin predates ks#262), so on a "
            f"venue serving raw bars it would build on them. Move the pin or unset the gate (#1124).")
    declared = _price_adjustments(feed)
    if declared is None:
        raise RuntimeError(
            f"strategies.CRSI_ENABLED is on but this node's data client declares NO price series "
            f"(`price_adjustments` undeclared) — CRSISHORT asks for {requested!r} bars and will not "
            f"guess. Declare it on the DataClientSpec (#1124).")
    if requested not in declared:
        raise RuntimeError(
            f"strategies.CRSI_ENABLED is on but this node's data client serves only "
            f"{sorted(declared)} — CRSISHORT asks for {requested!r} daily bars. See kumo-trading-platform issue 1124 "
            f"/ #854 before enabling this lane on a venue that cannot serve them.")
    # The IBKR data client used to declare `daily_bars_cover="extended"` (pre/post-market folded
    # into the daily OHLCV, #616) while the lab's panel is RTH SIP daily — prior close, high, low and
    # volume all differ. Refused unless the deviation is STATED in settings, then said at WARNING.
    ...
    if borrow_rates is None:
        if not _borrow_gate_off():
            raise RuntimeError(
                ...
                "(kumo-trading-platform issue 857) and `strategies.CRSI_BORROW_GATE_OFF` is not set — a locate "
    ...
    async def _read_state():
        """A FRESH lifecycle read per session. AN ABSENT ROW IS TRADING (2026-08-19) — the
    ...
    async def _read_budget():
        ...
        (issue 173) and capital at risk is a magnitude. A missing price RAISES rather
    ...
    async def _write_claim(symbol: str, post_trade_qty: float, px):
        ...
        is what makes that legible to every other reader (issue 173)."""
    ...
    try:
        strategy = CrsiShortStrategy(
            ...
            # THE RESOLVED LADDER, NOT THE BUILT-IN (#131). kumo-trading-strategies refuses at CONSTRUCTION when
            ...
            **_live_reread_kwargs(
                ...
                # SHADOW BY DECLARATION (issue 141): the adapter refuses to construct a lane whose
                ...
                # THE PATH IS BUILT, SO THIS DEPLOYMENT TRADES (issue 131 landed at 7fae46b).
                ...
                # platform issue 853's first phase and the safest way to validate a new lane against a venue.
        ...
        # THE ADAPTER REFUSES AT CONSTRUCTION WITH ValueError (kumo-trading-strategies crsi_short.py:424
        # adjustment, :432 min_warm, :441 borrow ceiling without a provider, :453 no slot reaches
        # the auction). `build_optional_strategy` reads a RuntimeError as a REFUSAL (records it,
        # degrades /health, the node boots) and anything else as a BUG (propagates, the node dies).
        # A refusal is a refusal whichever class the adapter picked — re-raised FROM the original
        # so the operator reads which line refused (#1123, ks review of #1125).
        raise RuntimeError(f"{STRATEGY_ID} refused at construction: {exc}") from exc
