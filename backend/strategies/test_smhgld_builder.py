"""SMHGLD-007 registers in the node behind `SMHGLD_ENABLED` (#965, cockpit half of kumo-strategies#177).

#953 delivered the executing gateway and left the lane "registered nowhere … the correct state until"
this: a builder that constructs the upstream `SmhGldSleeveStrategy` (kumo-strategies ≥ 06f3055)
...
def smhgld_installed() -> bool:
    """Whether the installed kumo-strategies carries the SMHGLD sleeve adapter (#177, 06f3055)."""
    try:
...
def _upstream_or_skip():
    try:
        ...
        pytest.skip("installed kumo-strategies predates the SMHGLD sleeve adapter (kumo-strategies#177, 06f3055)")
...
def test_the_builder_REFUSES_by_name_when_the_installed_strategies_lack_the_sleeve(monkeypatch):
    ...
    def _no_upstream(name, *a, **k):
        if name.startswith("kumo_strategies.runtime.nautilus.smhgld_sleeve"):
            raise ImportError("no module named smhgld_sleeve (pin predates kumo-strategies#177)")
