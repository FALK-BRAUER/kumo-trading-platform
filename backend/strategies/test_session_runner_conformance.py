"""The injected-runner seam: cockpit's `SessionGateway` must satisfy how kumo-trading-strategies CALLS it.
...
agree, so on 2026-08-16 kumo-trading-strategies `076035c` added a `slot=` kwarg to the call, cockpit's signature
...
binding the call site against COCKPIT's implementation. (kumo-trading-strategies made this point; it is the
reason this test binds arguments rather than comparing two signatures.)

WHY NOT BIND `_session_coro` DIRECTLY
-------------------------------------
It is upstream's private method and would break on rename. kumo-trading-strategies is exporting a
`SessionRunner` Protocol (issue 45); when it lands, import it here and conform to it
...
# Every shape kumo-trading-strategies calls an injected runner with, as of 2026-08-18. Grepped from their
# source, not recalled:
