...
exception. That happened on staging-ibkr on 2026-08-24 — a deploy re-seeded the settings volume from
...
ON IBKR NEITHER PROTECTION HOLDS. The account reports 999,216.15 SGD, and the budget gate is not in the
...
The fix is one line and it lives in kumo-strategies: `allocated_equity if ... is not None else ...`,
...
def test_a_zero_target_now_ARRIVES_as_zero_but_the_hazard_is_only_HALF_closed(monkeypatch):
    ...
    in the INSTALLED kumo-strategies is still `allocated_equity or broker.equity()`, and 0.0 is
...
def test_ZERO_IS_NO_LONGER_FUSED_WITH_UNSET_in_the_installed_runner(module):
    ...
    kumo-strategies OLDER than the deployed pin, so the whole local suite had been measuring a
    ...
    `0.0` no longer falls through to the account. That mattered on staging-ibkr, where BCTROT-004
    ...
    src = inspect.getsource(importlib.import_module(
        f"kumo_strategies.runtime.executor.{module}"))
...
def test_a_ZERO_allocation_reaches_the_runner_AS_ZERO_and_does_not_become_the_account():
    ...
    account_equity = 999_216.15          # the live staging IB account, in SGD
...
def test_an_UNSET_allocation_still_falls_back_to_the_account_which_is_DELIBERATE():
    ...
    kumo-strategies documents as sizing off the account. Only ZERO was ever the defect.
    """
    from kumo_strategies.runtime.executor.runner import RiskLimits

    account_equity = 999_216.15
    unset = RiskLimits()
