...
`ledger-tool/tools/{rotation_read,grade_full}.py` and into kumo. A port is exactly the case where reading
...
ORIGINAL_DIR = os.environ.get("ROTATION_ORIGINAL_DIR", "~/projects/ledger-tool/tools")
...
#: inert once the mount is gone, and it is gone — nobody can re-run them. kumo-trading-strategies made the
