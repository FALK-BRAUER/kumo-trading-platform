...
Measured on an Alpaca paper instance, BCTROT-004 GMAB, 2026-09-09 19:40:19Z. Nautilus's position reconciler pulled three
...
#: The row as it stood before the exit — the backup table's copy of it (issue 124).
    ...
    def sessionmaker(self):
        ...
        class _Session:
            ...
            async def execute(self, stmt, params=None):
                # `pg_advisory_xact_lock` (issue 124): modelled as a per-key wait until the
...
def test_the_ledger_double_CAN_resurrect_a_row_when_commits_land_out_of_order():
    ...
    async def bare(ledger, stmt):
        ...
        # their writes since issue 124 and would make this fixture test pass vacuously.
...
def test_the_store_upper_cases_the_symbol_so_two_casings_are_ONE_key():
    ...
    casing is pinned where the lock lives — kumo-trading-strategies `test_claim_writes_are_serialised.py`
