...
measured nothing. RED HERE MEANS: align the venv to a kumo-trading-strategies revision that carries
`runtime/nautilus/crsi_short.py` and `strategies/crsi_short.LOCATABLE` (the branch to be pinned
when issue 123 lands on main).
"""
...
def test_the_installed_kumo_strategies_carries_the_CRSISHORT_adapter():
    ...
    assert crsishort_installed(), (
        ...
        f"is on the wrong branch, or the pin predates issue 123. Every CRSISHORT test in "
