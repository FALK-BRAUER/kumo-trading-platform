"""Deriving QC345's live ranking universe from Alpaca (#324, kumo-strategies#43).
...
--------------------------------------------------------
kumo-strategies#43 measured, on local evidence:
...
#: The floors measured in kumo-strategies#43. Used to VERIFY a fetch, never to size one. See above.
...
def substrate(assets, cfg) -> list[str]:
    ...
    if not (low <= len(out) <= high):
        ...
        _log.error(
            ...
            "(kumo-strategies#43) — the asset filter may not match the researched one",
...
def _check_floors(diag: dict) -> None:
    """Compare what the selection needed against the floors measured in kumo-strategies#43.
