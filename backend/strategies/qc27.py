...
def _allocated_equity() -> float | None:
    ...
    NONE IS NOT A REFUSAL, and cockpit must not pretend it is. kumo-strategies confirmed (be244d9,
...
def _limits_for_session(limits):
    ...
    if "allocated_equity" not in getattr(type(limits), "__dataclass_fields__", {}):
        ...
        _log.warning("%s: installed kumo-strategies RiskLimits has no `allocated_equity` — sizing "
...
def _decision_slot() -> str:
    ...
    this file is a second derivation of a fact kumo-strategies already owns — and the two drifting
...
def _slot_and_offset_from_settings() -> tuple[str, int]:
    ...
    kumo-strategies rather than spelling it here — and the settings-vs-journal half still open: the
...
def build_qc27_strategy(*, feed=None):
    ...
    async def _read_state():
        ...
        AN ABSENT ROW IS TRADING (Operator, 2026-08-19), the same rule `QC345SessionGateway._lifecycle`
    ...
    strategy = QC27RotationStrategy(
        ...
        # pins kumo-strategies by revision and the seam arrived in db25413. See `slot_reader`.
