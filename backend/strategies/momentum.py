...
class SessionGateway:
    ...
    def __init__(self, sm, pool, journal, jobs, cfg, broker, limits, strategy_id="MOMENTUM-002",
        ...
        # exists (#199 A). The runner lives in kumo-strategies and must not import a cockpit module;
    ...
    def _limits_for_session(self):
        ...
        try:
            ...
            # not express a zero at all. Third site of one defect — kumo-strategies had the same
            # `or` at two (be244d9).
        ...
        if target <= 0:
            ...
            # whole-account fallback happen. kumo-strategies be244d9 fixed that: 0.0 now MEANS zero
            # and the lane declines to enter. So a zero is passed through as a zero.
            #
            # WHY THIS IS SAFE, not merely bad (kumo-strategies review): a strategy wound to target=0
            ...
            target = 0.0
        # THE FIELD MAY NOT EXIST. `RiskLimits` gained `allocated_equity` in kumo-strategies; this repo
        ...
        if "allocated_equity" not in type(self._limits).__dataclass_fields__:
            _log.warning(
                "%s: installed kumo-strategies RiskLimits has no `allocated_equity` — sizing off the "
    ...
    def _opens_kwarg(self, opens) -> dict:
        ...
        CONDITIONAL SO COCKPIT SHIPS FIRST. `opens` arrives from kumo-strategies#118 and today's
        ...
        _log.error(
            ...
            "decision journal says. Align the kumo-strategies pin (needs #118 or later).",
            self._strategy_id)
        return {}
    ...
    async def run(self, panel, session: str, jobs=None, slot: str = "open+5m", opens=None):
        ...
        `slot` is DEFAULTED and must stay that way. kumo-strategies calls this from two places with
        ...
        than a detail. kumo-strategies#118 calls `self._runner.run(..., slot=slot, opens=opens)`;
        ...
        asserts the outbound half; kumo-strategies' runner-protocol test binds the runners in THAT
...
def _resolve_claims(claimed, cache, strategy_id: str, _sink=None) -> list:
    ...
    # actually moved. Measured on the live book: 31 of 59 symbols are held by more than one lane, and
...
def live_config(overrides_prefix: str = "MOMENTUM"):
    ...
    hardcoded copy of these values living in kumo-strategies — which proves nothing about what ships,
    ...
    SAN UBS are ADRs, so roughly a quarter of the names are affected. George holds them deliberately,
    so trading them is defensible -- silently trading them while the config says otherwise is not.
    Supply a classifier to turn the gate back on. (Tracked as B3 on kumo-cockpit#197: the validated
    ...
    *moved* these would silently shrink the live portfolio from 8 names to 5 on the two lanes holding
    ...
    return MomentumRotationConfig(
        ...
        # the live evidence sets 8/5 explicitly (kumo-strategies
...
def decision_slots_from_settings(strategy_id: str, default: tuple[str, ...]) -> tuple[str, ...]:
    ...
    NOT the generated `qc345` domain (`api/settings/generated.py`). That walks kumo-strategies' config
...
def build_bctrot_strategy(feed=None):
    """BCTROT-004 — the same rotation decided at midday and near the close (kumo-strategies#32).

    Operator, 2026-08-16: register it and transition half of MOMENTUM's budget to it; both run alongside
    ...
    REGISTERING IT LETS IT TRADE. An absent lifecycle row reads as TRADING (Operator, 2026-08-19) --
...
def bctrot_config():
    ...
    return replace(
        ...
        # question stays open behind kumo-strategies#115.
...
def _build_rotation(strategy_cls, *, strategy_id: str, identity: dict, claims_from: str | None,
    ...
    async def _prepare():
        ...
        # MOMENTUM-002's name (kumo-strategies#54). A default that is right for exactly one caller is
        ...
        # changes: George closes a name and it leaves. If we still hold it, subscribing only to the
