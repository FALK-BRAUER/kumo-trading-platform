"""SMHGLD — the fixed-weight SMH/GLD sleeve's session gateway: delta execution (kumo-trading-platform issue 953).

WHAT THIS MODULE IS. The cockpit half of issue 177. The adapter (`runtime/nautilus/
...
per symbol per session, sells first, and refuses by name whatever it cannot size or send. Claims are
synced later from terminal order events and the lane's actual Nautilus book. The strategy logic lives
upstream (`kumo_strategies.strategies.smhgld_sleeve`);
nothing here decides, targets, rounds or orders.

THE BOUNDARY (2026-09-11: "double fix then" — the executing half is LANE-AGNOSTIC):
  * `strategies/delta_execution.py` EXECUTES a plan: sequential submission in the plan's order,
    ...
    buys budget-gated and sells never, every refusal coded, and no submit-time claim. It takes
    `tuple[Order, ...]`, a submit seam, a claim writer, a sleeve and prices, and
...
a name is satisfied by anything that spells it (platform issue 955's shape). The token's `means` includes
...
FOUR THINGS NOT RE-DERIVED HERE: the ordering (verified, never re-sorted — refused whole), the
...
itself supports fractional shares; the constraint is ours, asserted, not assumed), the target, and
the delta. The claim value is not planned here at all: terminal order events sync the actual cache
quantity via `sync_claim`.
...
a flatten, a corporate action or a reconciliation flatting order) is kumo-trading-platform issue 961 — the
...
class SmhgldSessionGateway:
    ...
    def __init__(self, *, journal, broker, read_state, plan, strategy_id: str,
        ...
        #: `async (symbol, actual_qty, px) -> None`: the event-sync writer. A precondition of SENDING:
        #: without it a terminal event cannot bring the claim back to the actual lane book.
    ...
    async def sync_claim(self, symbol: str, qty: int, px: float | None) -> None:
        """The claim, brought back to what this lane's OWN Nautilus cache position says (#829).

        Called from the adapter's terminal order handlers with `(symbol, signed lane quantity,
        fill price or None)`, positionally. The builder wires `write_claim` to the shared
        `api.claims_sync.sync_claim_from_book` predicate; this method is the adapter-facing contract.
        """
        if self.write_claim is None:
            _log.warning("%s: claim for %s not synced to %s — no claim writer wired",
                         self.strategy_id, symbol, qty)
            return
        try:
            await self.write_claim(symbol, qty, px)
        except Exception as exc:  # noqa: BLE001 — never raise into a Nautilus event handler
            _log.warning("%s: claim for %s not synced to %s (%r)",
                         self.strategy_id, symbol, qty, exc)
    ...
    async def run(self, panel, session: str, jobs=None, slot: str | None = None):
        ...
        if state.may_submit_exits and not state.may_submit_entries:
            why = (f"lifecycle {state.value}: this gateway sends nothing while the lane is being wound "
                   f"down — liquidate_lane (kumo-trading-platform issue 922) owns it")
            await self._row("risk", "liquidating", why, session=session, lifecycle=state.value,
                            mechanism="liquidate_lane", issue="kumo-trading-platform issue 922")
            return _result(decided=True, submitted=0, blocked=why)

        acting = state.may_submit_entries
        if acting and self.write_claim is None:
            why = ("no claim writer is wired — terminal order events cannot sync the lane's actual "
                   "book to the claims ledger; the whole session is refused")
        ...
        # workaround: the real fix is upstream (issue 186 — `order_plan` rounding buys
        # toward the sleeve), and this reserve leaves with it. A NOTE THAT STOOD HERE WAS FALSE and is
        # recorded rather than deleted: it said ibkr-paper's sleeves all read actual=0 and that this
        # gateway would refuse every session there. Measured from the running instance (`load_book`
        # inside kumo-ibkr-paper-api-1, 2026-09-11 11:46Z): all six lanes are target=100,000.00 and
        ...
        # headerless `select *` read column-order backwards. The basis on ibkr-paper is 100,000, not 0.
        import math
...
#: moves it to TRADING deliberately. An absent row would read TRADING (2026-08-19).
...
def build_smhgld_strategy(*, feed=None):
    ...
    try:
        ...
        raise RuntimeError(
            f"strategies.SMHGLD_ENABLED is on but the installed kumo-trading-strategies has no SMHGLD sleeve "
            f"adapter ({exc}) — the pin predates issue 177 (06f3055). Unset the gate or "
    ...
    async def _write_claim(symbol: str, actual_qty: float, px):
        """The claim SET to the actual terminal-event book quantity through the one predicate every
        gateway shares (#950)."""
        from api.claims_sync import sync_claim_from_book
        await sync_claim_from_book(journal, STRATEGY_ID, symbol, int(actual_qty), px)
    ...
    strategy = SmhGldSleeveStrategy(
        ...
        # HISTORY, OR THE LANE CAN NEVER BE WARM (issue 200). `on_start` subscribes to
        ...
        # Measured live on ibkr-paper 2026-09-11: SMHGLD armed, fired its rung, and returned silently
