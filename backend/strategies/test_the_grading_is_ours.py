...
REPLACES `test_ledger_tool_optional.py`, which pinned the opposite property — that the grading was a
...
correct about the code and wrong about the intent:
    ...
    test_an_instance_can_declare_where_ledger_tool_is
    test_an_ABSENT_mount_raises_the_CONFIGURATION_error_not_ImportError
    test_LedgerToolUnavailable_is_not_confusable_with_a_real_import_failure
...
ibkr-paper-retired sat in that state for days showing "No rotations to show", and it read as deliberate
...
def test_NOTHING_in_the_backend_reaches_a_mounted_checkout_to_grade():
    ...
    # THE MECHANISM, NOT THE WORD. A first version matched the string `/ledger-tool/tools` and flagged
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Call):
                ...
                if (not is_test
                        ...
                        # exactly how `/ledger-tool/tools` got on the path.
                        and any(isinstance(a, ast.Constant) and isinstance(a.value, str)
                                for a in node.args)):
                    offenders.append(
                        f"{path.relative_to(BACKEND)}:{node.lineno} sys.path <- literal path")
                # os.environ.get("KUMO_LEDGER_TOOL_TOOLS")
                for arg in node.args:
                    if isinstance(arg, ast.Constant) and arg.value == "KUMO_LEDGER_TOOL_TOOLS":
                        offenders.append(
                            f"{path.relative_to(BACKEND)}:{node.lineno} reads KUMO_LEDGER_TOOL_TOOLS")
...
def test_EVERY_INSTANCE_can_grade_a_rotation():
    ...
    assert not hasattr(adapter, "LedgerToolUnavailable"), (
