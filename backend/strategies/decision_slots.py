...
`decision_slots_from_settings`: `if not raw: return default`; kumo-strategies' rotation does the
...
def lane_declared_slots(strategy_id: str) -> tuple[str, ...] | None:
    ...
    lane over. kumo-strategies' own docstring records the same literal drifting once before.
    ...
    try:
        ...
        if strategy_id == "SMHGLD-007":
            # Read off the upstream adapter's SIGNATURE (kumo-strategies#177), never typed: the
...
BUILTIN_SLOTS: dict[str, tuple[str, ...]] = {
    ...
    # SMHGLD-007 (kumo-strategies#177): decide at close-20m, fill market-on-close (cockpit#965).
