...
`/fintrack/tools`, behind `KUMO_FINTRACK_TOOLS` and a `FintrackUnavailable` exception. So an env var
could switch the market view off, and `KUMO_FINTRACK_TOOLS=/nonexistent` read as deliberate
configuration rather than a broken feature — which is exactly how staging-ibkr showed
