"""The INSTALLED kumo-trading-strategies must accept every kwarg cockpit's builders pass it.
...
backend venv carried a kumo-trading-strategies where NEITHER `QC27RotationStrategy` nor
...
`kumo-trading-platform/instances/versions.lock`. This asserts the narrower, local property: the strategies
...
RED HERE MEANS: `uv pip install -e ../../kumo-trading-strategies` (or align the venv to the deployed pin)
...
INSTALLED = [
    ...
    # this file covered only the two above until the kumo-trading-strategies peer pointed out the gap
...
def _cls(module: str, name: str):
    ...
    if name == "CrsiShortStrategy":
        ...
        if not crsishort_installed():
            pytest.skip("CRSISHORT adapter absent from the installed kumo-trading-strategies — the pin "
                        "predates issue 121 (loud guard is the strict xfail beside it)")
...
def test_the_installed_class_ACCEPTS_the_live_slot_rereader(label, module, name, kwargs):
    ...
    assert not missing, (
        ...
        f"(`uv pip install -e ../../kumo-trading-strategies`) before trusting any result from this suite."
    )
...
#: Names cockpit reads off a kumo-trading-strategies object with `getattr(..., default)`, so a rename
...
#: `hasattr(QC27RotationStrategy, "is_running")` is True no matter what kumo-trading-strategies does —
...
#: an attribute silently moving between kumo-trading-strategies' mixin and Nautilus's base means cockpit is
...
GUESSED_ON_STRATEGY = [
    ...
    ("is_armed", "RegistrationMixin",
     "engine_node.py:365 reads the lane's own arming answer. kumo-trading-strategies owns it, so a rename "
...
def test_every_attribute_cockpit_GUESSES_on_a_runner_actually_exists(
    ...
    """These ARE existence checks, legitimately: the runners are kumo-trading-strategies' own classes with no
