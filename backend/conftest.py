...
def pytest_sessionstart(session):
    """THE PIN ANCHOR, ASSERTED INSIDE THE SUITE (kumo-trading-platform issue 947). `merge_gate --against-pin` exports
    ...
    resolved — two invocations, two facts (l21wvpmj, cross-repo review; issue 162 was the
...
def _skip_absent_captured_fixtures(items):
    """A `captured_fixture(<name>)` test is SKIPPED, by name, where `api/fixtures/<name>/` is absent (#1045).

    Those directories are production captures of the paper stack's Redis and venue state — the real
    book, with the real account ids inside base64 msgpack blobs that no grep sees. They are excluded
    from the public export (`.public-exclude`), so in the public tree the tests that read them cannot
    run. A skip that NAMES the directory and the reason is honest; an ImportError or a FileNotFoundError
    in CI would read as a broken test rather than as an absent private input.
    """
    from pathlib import Path

    root = Path(__file__).parent / "api" / "fixtures"
    for item in items:
        for m in item.iter_markers("captured_fixture"):
            name = m.args[0]
            if not (root / name).is_dir():
                item.add_marker(pytest.mark.skip(reason=(
                    f"production-captured fixture api/fixtures/{name}/ is absent — it is private and not "
                    f"exported (#1045); this test runs only in the private tree")))
...
def pytest_collection_modifyitems(config, items):
    ...
    _skip_absent_captured_fixtures(items)
...
#
# `kumo-trading-strategies` is a separate PRIVATE repo. `deploy/Dockerfile.backend` never installs it from git
...
if not _strategies_available():  # pragma: no cover - depends on the install, not on a code path
    ...
    print(
        f"\n[conftest] kumo-trading-strategies is NOT installed — deselecting {len(collect_ignore)} test files "
...
def _no_pool_cache_leaks_between_tests():
    ...
    Since kumo-trading-platform issue 511 `providers.ibkr.build`/`build_data` union `exec_pool_source` into
...
# ---- the INSTALLED strategies, read through their shims (test support, not a mechanism) ----------
"""(conftest, so the orphan guard does not count it as production.) Where an INSTALLED kumo_strategies module really lives.

The strategies package moved every lane and session runner into `strategies/<name>/` and left a
re-export shim at each old path (`runtime.executor.pgrunner` → `strategies.momentum_rotation.runner`,
`runtime.nautilus.<lane>` → `strategies.<name>.nautilus`). Cockpit's seam tests read those modules'
SOURCE — `inspect.getsource`, `ast.parse(module.__file__)` — and a shim's source is three lines, so a
scan over it finds nothing and passes on nothing (the exact shape the risk-limit and sizing-hazard
tests exist to catch). Every seam test resolves the module through here.

`real_module(dotted)` imports the old path and follows the exported objects to the module that
DEFINES them; a module that defines its own objects comes back unchanged, so the helper is inert on a
pin without shims.
"""
def real_installed_module(dotted: str) -> ModuleType:
    """The module a shim stands for. A shim is `from <real> import *` plus `__all__`, so the answer is
    read off the module's own source — the one star-import — not guessed from the exported objects
    (the real module re-exports its own imports, so an object-majority vote lands on the wrong file).
    A module with no star-import is not a shim and comes back unchanged."""
    import ast
    import inspect
    ...
    if len(stars) != 1 or not stars[0].startswith("kumo_strategies"):
        return mod
    return importlib.import_module(stars[0])
