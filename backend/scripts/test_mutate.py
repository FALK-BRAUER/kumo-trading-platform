...
def test_an_AMBIGUOUS_anchor_is_refused(src):
    """THE ONE THAT ACTUALLY BIT kumo-trading-strategies. `replace(a, b, 1)` takes the FIRST occurrence, so a
...
def test_a_SEMANTIC_no_op_is_APPLIED_because_no_string_harness_can_see_it(src):
    ...
    `"" or X` evaluates to `X` — the second kumo-trading-strategies case. The FILE changes and the BEHAVIOUR
