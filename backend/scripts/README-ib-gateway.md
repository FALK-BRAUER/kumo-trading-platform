...
The script's container name and host port are **fixed**. `alpaca-paper` and `ibkr-paper-retired` cannot each
...
```
make up INSTANCE=ibkr-paper-retired        # instance.env sets KUMO_EXEC=ibkr
```
