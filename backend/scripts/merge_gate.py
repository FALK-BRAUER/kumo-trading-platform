...
THIS IS THE PERMANENT GATE. GitHub Actions will not be funded (Operator, 2026-09-11). CI did one thing the
...
def strategies_remote(pyproject: Path) -> str | None:
    """The remote on the SAME line as the pin: `kumo-strategies @ git+<url>@<sha>` -> `<url>`."""
...
def pin_run(python: Path, pyproject: Path, suite_cmd: str, cwd: Path, *,
    ...
    if not sha or not remote:
        raise Refused(f"no kumo-strategies pin line in {pyproject} — the pin run has nothing to test")
