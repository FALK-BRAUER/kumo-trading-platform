#!/usr/bin/env bash
# Launch the kumo-cockpit ENGINE process (issue #20) — the standalone Nautilus trading node.
...
#                                                 alpaca-key-paper / alpaca-secret-paper) — the active
#                                                 [data].provider (#23).
#         DATABENTO_API_KEY  — Databento key (keychain: databento-kumo-cockpit) — only if that provider
...
export APCA_API_KEY_ID="$(read_secret alpaca-key-paper)"
...
export DATABENTO_API_KEY="$(read_secret_opt databento-kumo-cockpit)"
