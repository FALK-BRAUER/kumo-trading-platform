...
This is the Cockpit↔kumo-strategies seam, and it is deliberately one-directional: Cockpit owns the
provider connection and the canonical `TICKER.MIC` instrument identity, so it exports the bars; the
strategy rules, the backtest, and every derived artifact stay in `kumo-strategies`. Nothing here knows
...
kumo-strategies `docs/momentum-rotation-literature.md`; the wire-level guard is
...
discontinuities. Detecting them is the consumer's data-quality gate (kumo-strategies Phase 1), not this
...
def build_manifest(
    *,
    ...
    return {
        ...
        "created_by": "kumo-cockpit backend/scripts/export_bars.py",
