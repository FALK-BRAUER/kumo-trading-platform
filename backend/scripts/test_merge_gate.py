...
def test_the_pin_is_read_from_pyproject(tmp_path):
    p = tmp_path / "pyproject.toml"; p.write_text('deps = ["kumo-strategies @ git+https://x/y.git@4d28488"]\n')
...
# Actions is unfunded (Operator, 2026-09-11); this gate is permanent. The venv's kumo_strategies is an EDITABLE
...
def test_the_strategies_remote_is_read_from_the_same_pin_line(tmp_path):
    ...
    p.write_text('deps = ["kumo-strategies @ git+https://github.com/X/kumo-strategies.git@4d28488"]\n')
    assert mg.strategies_remote(p) == "https://github.com/X/kumo-strategies.git"
    assert mg.pinned_strategies_rev(p) == "4d28488"
...
def test_pin_run_REFUSES_when_the_interpreter_does_not_resolve_the_pin_at_the_checkout(tmp_path, monkeypatch):
    ...
    p = tmp_path / "pyproject.toml"; p.write_text(f'deps = ["kumo-strategies @ git+file://{src}@{one[:7]}"]\n')
