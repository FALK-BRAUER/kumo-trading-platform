...
Operator, 2026-08-21: "We have that problem again and again."
...
thing and the machine does another, and nothing reports it:
  ...
  * The same day: the engine gained a `fintrack/tools` mount and the api lost `KUMO_ROTATION_PATH` and
...
def test_a_CHANGED_compose_config_is_reported_as_stale():
    """A mount, env var or command edited on disk and never applied. The engine's `fintrack/tools`
...
# every per-alert switch was on. compose.paper.yml passes `${FINTRACK_TELEGRAM_BOT_TOKEN:-}`, and that
...
def test_the_fixture_can_actually_see_the_compose_file():
    ...
    assert "FINTRACK_TELEGRAM_BOT_TOKEN" in _compose_env_vars("compose.paper.yml")
