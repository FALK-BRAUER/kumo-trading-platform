...
- `run-api.sh` — launch the FastAPI/Nautilus backend with secrets injected from the macOS keychain
  (`DATABENTO_API_KEY` ← service `databento-kumo-cockpit`). Use this instead of bare `python -m api` once
...
- `export_bars.py` — freeze a **raw (unadjusted)** historical OHLCV snapshot + manifest for out-of-tree
  backtesting (`kumo-strategies`). Cockpit exports the data and owns the `TICKER.MIC` identity; strategy
  rules and backtests live in `kumo-strategies`. Reads Alpaca keys from the environment only — never the
...
size against the ~5,523 measured in kumo-strategies#43, what the selection actually needed, and
