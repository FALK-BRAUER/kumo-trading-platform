"""`slot` on exec_action_log, so a session can hold more than one decision (issue 29).
...
Cockpit owns the migration; kumo-trading-strategies owns the model (`runtime/executor/store.py`). This mirrors
...
def _table_exists() -> bool:
    ...
    IT IS NOT OURS — kumo-trading-strategies declares it and its `create_all` builds it; no migration in this
...
def upgrade() -> None:
    if not _table_exists():
        # Fresh database: kumo-trading-strategies' create_all builds it from a model that already declares
        ...
        print(f"alembic 0015: {_TABLE} does not exist yet — kumo-trading-strategies' create_all owns it and "
