...
positions. The lifecycle state machine lives in Python in kumo-trading-strategies and was never consulted,
because nothing on that path goes through it: cockpit only ever READS this table, and kumo-trading-strategies
...
real gate is now kumo-trading-strategies' boot-time `dry_run`, which runs the full decide->size->form-order
...
def upgrade() -> None:
    if not _table_exists():
        # Fresh database: kumo-trading-strategies' create_all builds `exec_strategy_state`. The trigger is
        ...
        print(f"alembic 0016: {_TABLE} does not exist yet — kumo-trading-strategies' create_all owns it. "
            ...
            IF NEW.state = 'TRADING' THEN
                ...
                USING
                    ...
                    HINT =
                        ...
                        'See kumo-trading-platform issue 443.';
            END IF;
            RETURN NEW;
        END;
        $$ LANGUAGE plpgsql;
