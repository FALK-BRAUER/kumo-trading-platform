...
data=$(tracked '\.(csv|parquet|feather|h5)$' | grep -vE '(^|/)tests?/fixtures/' || true)
...
x=$(( ${#SPEC[@]} - 1 ))
echo "check-public-tree: 8 checks, 0 hits, $n tracked files scanned, $x exclusion(s) from .public-exclude${KUMO_PUBLIC_DENYLIST:+, denylist $(wc -l < "$KUMO_PUBLIC_DENYLIST" | tr -d ' ') term(s)}"
