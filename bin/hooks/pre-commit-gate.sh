...
# backend pytest
if ! ( cd backend && uv run --no-sync pytest -q -x ); then
  echo "commit gate: backend pytest REFUSED — fix it, do not bypass the hook" >&2
  exit 2
fi

# ui vitest
if ! ( cd ui && npm test --if-present --silent ); then
  echo "commit gate: ui vitest REFUSED — fix it, do not bypass the hook" >&2
  exit 2
fi
