 ...
 *
 * "blue_flag" — George's (Blue Cloud Trading) 8-condition entry checklist. Definition resolved from
 * kumo-trader's `scanner/ichimoku.py:145-155` + its `CLAUDE.md` (documented as stable, Phase 1 "done" —
 ...
 * kumo-trader's own `Close.iloc[-1]` semantics — keeps the checklist stable between renders rather than
...
const ADX_PERIOD = 9; // George's setting (playbook + kumo-trader default), not the textbook 14
const ADX_MIN = 20; // "no trend" floor, kumo-trader `scanner/ichimoku.py:150`
...
registerChecklist({
  ...
  // Matches kumo-trader's exact tier scheme (`scanner/ichimoku.py:158-166`), including the
