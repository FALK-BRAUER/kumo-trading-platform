...
import { cellHeadline, attribute, dayMove, dayMoveByStrategy, deployedValue, money, securedValue, periodRealized as periodRealizedOf, periodRealizedPartial, periodHorizonTs, periodBrokerRealized, periodUnclaimedRealized, readDayMove, rowPeriodRealized, periodRealizedByStrategy, visibleBooks, type Book, type CellHeadline, type DayMove, type SessionRealizedLike } from "@/tiles/managed-portfolio/books";
...
function BookCell({
  ...
  /** The exact headline included in the panel-level net reconciliation. */
  headline: CellHeadline;
}) {
  ...
  // `data-lane` is the cell's stable handle for the seam test that pins ONE rule across both tiles
  // (#1133): lane ORDER and neighbours change, the label attribute does not.
  return (
    <div className="bg-surf px-3 py-2" data-lane={label}>
...
export function BookTile({ data, status: srcStatus }: TileProps<BookConfig>) {
  ...
  //
  // 2026-08-27: "i have no stats. it does not help. cannot run like this." An em dash on
  ...
  const displayRows = rows.map((r) => {
    const periodRealized = rowPeriodRealized(tradesFrame, period, r.label);
    const unrealizedDelta = windowDelta(r.book.unrealized, windowBase?.by_period?.[period], r.label);
    const rowNet = windowNet(r.book.marketValue, windowBase?.net?.[period], r.label);
    const rowDay = dayByStrategy.get(r.label);
    return {
      ...r,
      day: rowDay,
      periodRealized,
      unrealizedDelta,
      net: rowNet,
      headline: cellHeadline(period, rowDay, periodRealized, unrealizedDelta, rowNet),
    };
  });
  const rowHeadlineValues = displayRows
    .map((r) => r.headline.value)
    .filter((v): v is number => typeof v === "number" && Number.isFinite(v));
  const rowHeadlineSum =
    displayRows.length > 0 && rowHeadlineValues.length === displayRows.length
      ? rowHeadlineValues.reduce((s, v) => s + v, 0)
      : null;
  const netResidual = net !== null && rowHeadlineSum !== null ? net - rowHeadlineSum : null;
  ...
  return (
    <div>
      ...
      >
          ...
          <Metric
            ...
            title={
              dUnrealized === null
                ...
                : `${unitRefusal("dUnrealized") ? `${unitRefusal("dUnrealized")} ` : ""}How much of NET came from the mark MOVING across ${periodLabel}, rather than from trades closed in it: NET − REALIZED — the BROKER's equity residual. The sub-line is the whole standing unrealized on what is held right now (gain accrued BEFORE this window included), and, when bases exist, Σ of the lanes' own "mark" figures — OUR marks against the end-of-day base. Two derivations of one window; they differ by basis noise and by anything the broker counts that our marks do not. The strategy grid reconciles its visible headline figures to this broker NET with a net residual line when the sources do not fully attribute.`
            }
          />
          <Metric
            label="Secured · now"
          ...
          <div className="grid grid-cols-2 gap-px overflow-hidden rounded-md border border-line bg-line sm:grid-cols-3">
            {displayRows.map((r) => (
              <BookCell
                ...
                periodRealized={r.periodRealized}
                unrealizedDelta={r.unrealizedDelta}
                net={r.net}
        ...
        {netResidual !== null && Math.abs(netResidual) > 0.005 && (
          <div className={`mt-1 px-1 font-mono text-[10px] ${tint(netResidual)}`}>
            net residual {fmtUsd(netResidual)} · {periodLabel}
            <span
              title={
                `Dollar gap between ${periodNetLabel(period)} and Σ of the strategy headline figures ` +
                `rendered above. Shown rather than forced into a lane: broker-period NET, day moves, ` +
                `partial realized, fees/cash adjustments, and venue mark fields can arrive from different ` +
                `planes. In $ mode, strategy headlines plus this residual equal the hero NET.`
              }
            >
              {" "}?
            </span>
        ...
        {displayRows.length > 0 && (unclaimedRealized ?? 0) !== 0 && (
