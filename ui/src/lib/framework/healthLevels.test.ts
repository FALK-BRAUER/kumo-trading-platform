...
const CASES: Record<Exclude<HealthLevel, "ok">, Input> = {
  ...
  reconcileDrift: {
    ...base,
    data: healthy({ reconcile_drift: [{ symbol: "HSBC", broker_qty: 93, platform_qty: 0 }] }),
  },
  ownershipViolation: {
    ...base,
    data: healthy({
...
describe("drift and ownership are two facts, not one slot (#807 item 3)", () => {
  it("the banner carries BOTH when both are present", () => {
    const both: Input = {
      ...base,
      data: healthy({
        reconcile_drift: [{ symbol: "PATH", broker_qty: 126, platform_qty: 0 }],
        ownership_violations: [
 ...
 *
 * Measured on ibkr-paper 2026-09-09: eleven divergent pairs, 1,504 shares, BCTROT-004 claiming
