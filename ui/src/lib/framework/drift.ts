...
export function classifyDrift(entry: DriftEntry): DriftKind {
  if (entry.broker_qty === 0 && entry.cockpit_qty === 0) return "insync";
  if (entry.cockpit_qty === 0) return "understated";
...
function mismatchDirection(e: DriftEntry): string {
  if (Math.sign(e.broker_qty) !== Math.sign(e.cockpit_qty)) return "opposite sides";
  return Math.abs(e.broker_qty) > Math.abs(e.cockpit_qty) ? "broker holds more" : "cockpit shows more";
}
...
export function driftMessage(drift: DriftEntry[] | null | undefined): string {
  ...
  if (phantom.length > 0) {
    clauses.push(`shown but not held at the broker: ${name(phantom, (e) => `${e.symbol} ${fmt(e.cockpit_qty)}`)}`);
  }
  if (mismatch.length > 0) {
    clauses.push(
      `quantity mismatch: ${name(
        mismatch,
        (e) => `${e.symbol} broker ${fmt(e.broker_qty)} vs cockpit ${fmt(e.cockpit_qty)} (${mismatchDirection(e)})`,
      )}`,
    );
  }
