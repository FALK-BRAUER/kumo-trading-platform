...
- [x] **Alpaca account usable from Singapore — CONFIRMED 2026-07-03.** Paper account `ACTIVE` ($100k cash), trading API + IEX data both reachable from SG on the paper key. SG eligibility risk cleared.
- [x] **Paper API key stored in keychain** — services `alpaca-key-paper` (Key ID, `PK…`) + `alpaca-secret-paper` (Secret). Adapter reads via Alpaca-standard env vars `APCA_API_KEY_ID` / `APCA_API_SECRET_KEY`. (Paper key is in the 2026-07-03 chat transcript → rotate in the Alpaca dashboard after wiring.)
...
- **Singapore account eligibility** — verify before investing in the adapter.
