...
and be steered. The first tenant is the momentum rotation in `kumo-trading-strategies` (backtested: +62.5%,
...
```
  SOURCES (automatic, machine-owned)
    ledger_book        nightly CSV from ledger-tool
...
symbol_pool_source(
  source        text not null,          -- 'ledger_book' | 'scanner' | ...
...
human layer on top. Analytics beyond what `kumo-trading-strategies` already computes (#112). Intraday
