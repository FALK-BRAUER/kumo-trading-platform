...
> Alpaca app that covers kumo-trading-platform's need (manual equity/ETF cockpit + light Nautilus automation),
> so we don't build?

## What kumo-trading-platform actually needs (the yardstick)
...
- **Bespoke analysis**: Ichimoku, ledger-tool gap-up / falling-knife, Blue-Flag 8-condition, sT10e entries.
...
### Options-specialist
PutHouse · Haiphen · PortfolioShield · AI Velocity Trading (30+ preset option strategies) · Market Gear · iVest Plus → **✗** (options = tradex domain, not kumo-trading-platform)
...
1. **Lama Trader** — a ready-made *modern Alpaca workstation* (charts + watchlists + portfolio + AI research + order entry + custom workspace). This is the single closest thing to kumo-trading-platform's **manual UI**. If the goal were "a nice manual terminal on Alpaca," this basically exists.
...
- Middle path worth considering: use **TradingView** for charting/idea-gen (you likely already do) and keep kumo-trading-platform focused on what's unique — the Nautilus lanes + custom risk/analysis — rather than re-building generic charting.

## Next step
Pick the framing: (a) kumo-trading-platform = generic manual terminal → evaluate **Lama Trader** hands-on before building more UI; (b) kumo-trading-platform = custom engine+analysis cockpit → keep building, Alpaca underneath (see `plan-alpaca-stack-migration.md`), and don't rebuild generic charting that TradingView already nails.
