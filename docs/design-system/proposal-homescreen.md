...
## 2. Why predecessor-repo's HomeDashboard is not the template

`predecessor-repo/ui/components/HomeDashboard.tsx`, 682 lines, is a finished answer to a **different**
...
4. Does `secured` survive without resting stops, or is it a predecessor-repo concept that does not
   translate?
...
## 8. What re-reading predecessor-repo's HomeDashboard changed

`predecessor-repo/ui/components/HomeDashboard.tsx:642-681`. §2 above tabulated its blocks but not their
...
| **Market** | **Open** — market-facing. predecessor-repo ran it on Gemini with Google Search grounding. Nothing here can verify a market claim, so it would be the one block on the page that cannot be checked |
...
| Sectors | **no data path** — predecessor-repo's rollup ran on its own scoring system; we have no scores. Portfolio-wide sector exposure is a different thing and lives in #12 / #80 |
