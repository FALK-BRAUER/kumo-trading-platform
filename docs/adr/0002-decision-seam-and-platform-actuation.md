...
| Entry sizing at less than a full book · session-dependent position cap · weights across `_resume` | kumo-trading-strategies | **OPEN by mutual agreement** — orthogonal to membership, not closed by it |
...
- **Deciders:** the operator. Negotiated with kumo-trading-strategies across a live peer session; every claim below traced
  to source in the running paper engine (Nautilus 1.229.0) or to this system's own logs and journal.
- **GitHub:** cockpit #346 #347 #348 #349 #350 · kumo-trading-strategies #45–#52
...
kumo-trading-strategies recommended dropping it until wired, and that is adopted: **the seam ships with the
...
kumo-trading-strategies `config.py:187`, *"the strategy has never had a stop (#30)"*; `stop_loss_atr` fires a full
...
| Claim | Source |
|---|---|
| The decision/actuation split already exists implicitly | kumo-trading-strategies `backtesting/runner.py:73` constructs the adapter with **no** `session_runner`; `_session_coro` — the method that broke — is never exercised in backtest |
...
**Deletion, kumo-trading-strategies:** claims tracking, orphan sweep, reconcile-vs-broker (`pgrunner.py:325-345`);
...
2026-08-19: *"do not forget the manual peak, flatten, stop and rebuy (which I never saw working)
...
- **The old path is DELETED, never left coexisting.** 2026-08-19: *"no. we need to clean up the old
...
because this session's exit has not filled. That is the session-dependent position cap kumo-trading-strategies
...
2. **It is PACED, and the pacing is a hard bound rather than an implication.** kumo-trading-strategies caught
...
The obstacle is concrete. `deploy/Dockerfile.backend:35-39` installs kumo-trading-strategies from a local build
...
`file:///tmp/kumo-trading-strategies` (#329, #J1).
...
2026-08-19: *"i said no fallback. it needs to work principled. no fallbacks, proper clean up"* and
*"fallbacks fire back."*
...
kumo-trading-strategies argued the protection reconciler is not a fallback in the rejected sense — an independent,
...
**Resolved after pushback, and the resolution is better than either starting position.** kumo-trading-strategies
...
Found by kumo-trading-strategies by tracing `pgrunner`'s reconcile, and verified here in the Nautilus source
...
- kumo-trading-strategies owns the fix: `pgrunner`'s reconcile needs "claimed, externally managed" as a state
...
2026-08-19: *"clean up plan included — so we don't change old code later."*
...
kumo-trading-strategies' refinement, and it is the right cut: `broker.py`'s `OrderResult`/`submit()` wrapper is
...
| Is `pyramid_watch` cockpit's alone? | **Yes** — checked here rather than assumed, because kumo-trading-strategies explicitly declined to answer it on either side's word. | Its trigger reads a `driver_instrument_id` (an operator-typed ticker), ADX rollover, own fade and breakout confirmation, with R derived once from the position's existing bracket stop. No ranking-engine input anywhere. |
...
Found 2026-08-19 by kumo-trading-strategies' code review of their own `#51` fix, and verified here in the
...
kumo-trading-strategies adopted is the one to copy rather than invent a second: an attempt component folded into
