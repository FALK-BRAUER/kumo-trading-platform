<!--
Provenance: drafted by fintrack HQ (2026-06-26), the operator-endorsed, relayed via the kumo-trader lead
...
# kumo-cockpit UI — Flexible Tile Framework Spec

*Author: fintrack HQ · 2026-06-26 · For: cockpit peer*
*Status: SPEC for steering. (Static-mockup gate dropped — the operator live-drives the look; framework + tiles per below.)*

## 0. Why this exists / what changed from kumo-trader

kumo-trader's UI is **NOT composable** — it's a tab-based monolith (`dashboard/page.tsx` → 10 hardcoded
...
kumo-cockpit replaces that with a **true composable tile framework**: tiles are registered units, data
...
All three live at `~/projects/kumo-trader/ui/components/`.
...
| `bct` | latest George/BCT post — flagged buys/sells | new | `bct` | — |
...
proven cockpit interaction from kumo-trader; keep it.
...
├──────────────────────────────────┼────────────────────────────────────┤
│ RISK  heat · sector · lane sleeves│ BCT FEED  George latest buys/sells  │
└──────────────────────────────────┴────────────────────────────────────┘
```
...
- For CURRENT position truth (live stops/qty/prices) broker fintrack (trading lane) rather than guessing.
